...
void UCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
	...
	if (!bUseRuntimeFloatCurves)
	{
		...
		View.Location = PivotLocation + PivotRotation.RotateVector(TargetOffset + FVector(-ZoomSize, 0.0f, 0.0f));
	}
	else
	{
		FVector LocalTargetOffset(0.0f);

		LocalTargetOffset.X = TargetOffsetX.GetRichCurveConst()->Eval(PivotRotation.Pitch);
		LocalTargetOffset.Y = TargetOffsetY.GetRichCurveConst()->Eval(PivotRotation.Pitch);
		LocalTargetOffset.Z = TargetOffsetZ.GetRichCurveConst()->Eval(PivotRotation.Pitch);
...
void UCameraMode_ThirdPerson::PreventCameraPenetration(const class AActor& ViewTarget, const FVector& SafeLoc,
                                                       FVector& CameraLoc, const float& DeltaTime,
                                                       float& DistBlockedPct, bool bSingleRayOnly)
{
	...
	for (int32 RayIdx = 0; RayIdx < NumRaysToShoot; ++RayIdx)
	{
		...
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bUseHit)
			{
				bHit = World->SweepSingleByChannel(Hit, SafeLoc, RayTarget, FQuat::Identity, TraceChannel, SphereShape,
				                                   SphereParams);
			}
#if ENABLE_DRAW_DEBUG
...
void UCameraMode_ThirdPerson::ZoomIn()
{
	ZoomSize += 100.0f;
	ZoomSize = FMath::Clamp(ZoomSize, ZoomSizeMin, ZoomSizeMax);
	//UToolFunctionLibrary::Debug(FString::Printf(TEXT("ZoomSize:%f"),ZoomSize));
}

void UCameraMode_ThirdPerson::ZoomAway()
{
	ZoomSize -= 100.0f;
	ZoomSize = FMath::Clamp(ZoomSize, ZoomSizeMin, ZoomSizeMax);
	//UToolFunctionLibrary::Debug(FString::Printf(TEXT("ZoomSize:%f"), ZoomSize));
...
void UCameraMode_ThirdPerson::SetZoom(float NewZoom)
{
	ZoomSize = NewZoom;
	ZoomSize = FMath::Clamp(NewZoom, ZoomSizeMin, ZoomSizeMax);
}

float UCameraMode_ThirdPerson::GetZoom()
{
	return ZoomSize;
}
