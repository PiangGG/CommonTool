...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnDeviceBoundsSet,bool, bSetMesh);

struct FGameplayTag;
UCLASS(Blueprintable,BlueprintType)
class COMMONTOOLCORE_API ABaseDevice : public AActor, public IDeviceInterface
{
	GENERATED_BODY()
...
protected:
	...
	virtual void ReDraw();
	UFUNCTION()
	virtual void OnDeviceBoundsSet(bool bSetMesh);
	...
	void SetMeshComponent(const FString &Path);

	void SetEnablePOI(bool bset);
	
	UFUNCTION(BlueprintCallable, Category = "Components")
	UActorComponent* FindComponentByName(FName ComponentName);
...
private:
	...
	UPROPERTY()
	FOnDeviceBoundsSet DeviceBoundsSet;
public:
