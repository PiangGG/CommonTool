...
DECLARE_DYNAMIC_DELEGATE_TwoParams(FDownloadCompleteDelegate, bool, bSuccess, const FString&, FilePath);

UCLASS()
class COMMONTOOLCORE_API UHttpSubSystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category="CommonToolCore|Http")
	...
	//游戏消息回调
	UFUNCTION(BlueprintNativeEvent, Category="CommonToolCore|Http")
	...
	//http 回调
	UPROPERTY(BlueprintAssignable, Category="CommonToolCore|Http")
	...
	void DownloadFileAsync(const FString& URL, const FString& SavePath, const FDownloadCompleteDelegate& DownloadCompleteDelegate);
	void OnDownloadComplete(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful, FString SavePath, FDownloadCompleteDelegate CompleteDelegate);
};
