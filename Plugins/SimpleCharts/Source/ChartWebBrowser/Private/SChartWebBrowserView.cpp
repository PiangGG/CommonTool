...
void SChartWebBrowserView::Construct(const FArguments& InArgs, const TSharedPtr<IWebBrowserWindow>& InWebBrowserWindow)
{
	...
	if (BrowserWindow.IsValid())
	{
		...
		{
			BrowserWindow->OnConsoleMessage().BindSP(this, &SChartWebBrowserView::HandleConsoleMessage);
		}
...
void SChartWebBrowserView::CloseBrowser()
{
...
#if WITH_CEF3
				.RenderTransform(this, &SChartWebBrowserView::GetPopupRenderTransform)
#endif
...
TOptional <FSlateRenderTransform> SChartWebBrowserView::GetPopupRenderTransform() const
{
	if (BrowserWindow.IsValid())
	{
...
void SChartWebBrowserView::SetParentWindow(TSharedPtr<SWindow> Window)
{
	SetupParentWindowHandlers();
	if (BrowserWindow.IsValid())
	{
...
void SChartWebBrowserView::SetBrowserKeyboardFocus()
{
	BrowserWindow->OnFocus(HasAnyUserFocusOrFocusedDescendants(), false);
}

void SChartWebBrowserView::HandleConsoleMessage(const FString& Message, const FString& Source, int32 Line, EWebBrowserConsoleLogSeverity Serverity)
{
