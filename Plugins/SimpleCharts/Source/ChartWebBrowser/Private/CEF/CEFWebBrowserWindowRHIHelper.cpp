...
FSlateUpdatableTexture *FCEFWebBrowserWindowRHIHelper::CreateTexture(void* SharedHandle)
{
	...
	if (FAILED(Hr))
	{
		UE_LOG(LogChartWebBrowser, Error, TEXT("FCEFWebBrowserWindowRHIHelper::CreateTexture() - - ID3D11Device::QueryInterface"));
		return nullptr;
	}
	...
	if (FAILED(Hr))
	{
		UE_LOG(LogChartWebBrowser, Error, TEXT("FCEFWebBrowserWindowRHIHelper::CreateTexture() - - ID3D11Device::OpenSharedResource"));
		return nullptr;
	}
...
void FCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture(void *SharedHandle, FSlateUpdatableTexture* SlateTexture, const FIntRect& DirtyIn)
{
	...
	if (FAILED(Hr))
	{
		UE_LOG(LogChartWebBrowser, Error, TEXT("FCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - ID3D11Device::QueryInterface"), Hr);
		return;
	}
	...
	if (FAILED(Hr))
	{
		UE_LOG(LogChartWebBrowser, Error, TEXT("FCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - ID3D11Device::OpenSharedResource"), Hr);
		return;
	}
	...
	if (FAILED(Hr))
	{
		UE_LOG(LogChartWebBrowser, Error, TEXT("FCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - ID3D11Texture2D::IDXGIKeyedMutex"), Hr);
		return;
	}
	...
	ENQUEUE_RENDER_COMMAND(CEFAcceleratedPaint)(
		[KeyedMutex, SlateTexture, Dirty, Tex](FRHICommandList& RHICmdList)
		{
			if (KeyedMutex)
			{
				...
				{
					UE_LOG(LogChartWebBrowser, Verbose, TEXT("FCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - failed getting sync"), E_FAIL);
				}
			}
			else
			{
				UE_LOG(LogChartWebBrowser, Verbose, TEXT("FCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - missing KeyedMutex"), E_FAIL);
				return;
			}
	});
#else
	UE_LOG(LogChartWebBrowser, Error, TEXT("FCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - missing implementation"));
#endif // PLATFORM_WINDOWS
#else
	UE_LOG(LogChartWebBrowser, Error, TEXT("FCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - unsupported usage, RHI renderer but missing engine"));
#endif // WITH_ENGINE
}
