...
public:
	FPosixSignalPreserver()
	{
		...
		for (uint32 i = 0; i < UE_ARRAY_COUNT(PreserveSignals); ++i)
		{
			...
			{
				UE_LOG(LogChartWebBrowser, Warning, TEXT("Failed to backup signal handler for %i."), PreserveSignals[i]);
			}
	...
	~FPosixSignalPreserver()
	{
		for (uint32 i = 0; i < UE_ARRAY_COUNT(PreserveSignals); ++i)
		{
			...
			{
				UE_LOG(LogChartWebBrowser, Warning, TEXT("Failed to restore signal handler for %i."), PreserveSignals[i]);
			}
		}
	}
	...
	, DefaultTranslucentMaterial(nullptr)
{
	...
	if (bAllowCEF)
	{
		// The FChartWebBrowserSingleton must be initialized on the game thread
		...
		{
			UE_LOG(LogChartWebBrowser, Error, TEXT("EpicWebHelper.exe not found, check that this program has been built and is placed in: %s."), *SubProcessPath);
		}
...
#if WITH_CEF3
void FChartWebBrowserSingleton::WaitForTaskQueueFlush()
{
...
TSharedPtr<IWebBrowserWindow> FChartWebBrowserSingleton::CreateBrowserWindow(const FCreateBrowserWindowSettings& WindowSettings)
{
	...
	if (bAllowCEF)
	{
		...
		if (WindowSettings.OSWindowHandle != nullptr)
		{
			...
			{
				UE_LOG(LogChartWebBrowser, Error, TEXT("Failed to get client rect"));
			}
		...
		if (WindowSettings.Context.IsSet())
		{
			...
			UE_LOG(LogChartWebBrowser, Log, TEXT("Creating browser for ContextId=%s."), *WindowSettings.Context.GetValue().Id);
		}
...
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_FChartWebBrowserSingleton_Tick);
...
#if WITH_CEF3
bool FChartWebBrowserSingleton::URLRequestAllowsCredentials(const FString& URL)
{
	FScopeLock Lock(&WindowInterfacesCS);
...
FString FChartWebBrowserSingleton::GenerateWebCacheFolderName(const FString& InputPath)
{
...
void FChartWebBrowserSingleton::ClearOldCacheFolders(const FString &CachePathRoot, const FString &CachePrefix)
{
#if WITH_CEF3
	...
	struct FDirectoryVisitor : public IPlatformFile::FDirectoryVisitor
	{
		...
		virtual bool Visit(const TCHAR* FilenameOrDirectory, bool bIsDirectory) override
		{
			...
			if (bIsDirectory)
			{
				...
				{
					UE_LOG(LogChartWebBrowser, Log, TEXT("Old Cache folder found=%s, deleting"), *DirName);
...
bool FChartWebBrowserSingleton::RegisterContext(const FBrowserContextSettings& Settings)
{
#if WITH_CEF3
	if (bAllowCEF)
	{
		...
		SchemeHandlerFactories.RegisterFactoriesWith(RequestContext);
		UE_LOG(LogChartWebBrowser, Log, TEXT("Registering ContextId=%s."), *Settings.Id);
		return true;
	}
#endif
	return false;
...
bool FChartWebBrowserSingleton::UnregisterContext(const FString& ContextId)
{
	...
	if (bAllowCEF)
	{
		UE_LOG(LogChartWebBrowser, Log, TEXT("Unregistering ContextId=%s."), *ContextId);
