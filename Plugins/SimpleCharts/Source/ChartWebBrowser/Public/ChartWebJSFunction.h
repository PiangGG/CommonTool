 ...
 * FChartWebJSFunction objects represent a JS function and allow calling them from native code.
 * FChartWebJSFunction objects can also be added to delegates and events using the Bind/AddLambda method.
 */
USTRUCT()
 ...
 *  UFUNCTIONs taking a FChartWebJSResponse will get it passed in automatically when called from a web browser.
 *  Pass a result or error back by invoking Success or Failure on the object.
 *  UFunctions accepting a FChartWebJSResponse should have a void return type, as any value returned from the function will be ignored.
