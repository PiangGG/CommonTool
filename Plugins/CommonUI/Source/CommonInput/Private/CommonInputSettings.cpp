...
void UCommonInputSettings::PostInitProperties()
{
	...
	if (CommonInputPlatformData_DEPRECATED.Num())
	{
		for (const auto& PlatformData : CommonInputPlatformData_DEPRECATED)
		{
			...
			if (UCommonInputPlatformSettings* Settings = UPlatformSettingsManager::Get().GetSettingsForPlatform<
				UCommonInputPlatformSettings>(PlatformData.Key))
			{
			...
			else if (PlatformData.Key == FCommonInputDefaults::PlatformPC)
			{
				...
				PCPlatforms.Add(
					UPlatformSettingsManager::Get().GetSettingsForPlatform<UCommonInputPlatformSettings>("Windows"));
				PCPlatforms.Add(
					UPlatformSettingsManager::Get().GetSettingsForPlatform<UCommonInputPlatformSettings>("WinGDK"));
				PCPlatforms.Add(
					UPlatformSettingsManager::Get().GetSettingsForPlatform<UCommonInputPlatformSettings>("Linux"));
