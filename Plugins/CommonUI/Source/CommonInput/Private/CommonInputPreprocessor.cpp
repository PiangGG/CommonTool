...
{
	for (uint8 InputTypeIndex = 0; InputTypeIndex < static_cast<uint8>(ECommonInputType::Count); InputTypeIndex++)
	{
...
bool FCommonInputPreprocessor::HandleAnalogInputEvent(FSlateApplication& SlateApp,
...
bool FCommonInputPreprocessor::HandleMouseButtonDownEvent(FSlateApplication& SlateApp,
...
bool FCommonInputPreprocessor::HandleMouseButtonDoubleClickEvent(FSlateApplication& SlateApp,
                                                                 const FPointerEvent& InPointerEvent)
{
	const ECommonInputType InputType = GetInputType(InPointerEvent);
	if (IsRelevantInput(SlateApp, InPointerEvent, InputType))
...
{
	TMap<FName, bool>& Reasons = FilterInputTypeWithReasons[static_cast<uint8>(InputType)];
	...
	InputMethodPermissions[static_cast<uint8>(InputType)] = ComputedFilter;
}

bool FCommonInputPreprocessor::IsInputMethodBlocked(ECommonInputType InputType) const
{
	return InputMethodPermissions[static_cast<uint8>(InputType)];
}

bool FCommonInputPreprocessor::IsRelevantInput(FSlateApplication& SlateApp, const FInputEvent& InputEvent,
                                               const ECommonInputType DesiredInputType)
{
	...
	if (SlateApp.IsActive()
		...
		|| (ICommonInputModule::GetSettings().GetAllowOutOfFocusDeviceInput() && DesiredInputType ==
...
void FCommonInputPreprocessor::RefreshCurrentInputMethod(ECommonInputType InputMethod)
{
		...
		&& UCommonInputPlatformSettings::Get()->CanChangeGamepadType())
	{
		...
		{
			if ((DeviceScope->InputDeviceName != LastSeenGamepadInputDeviceName) || (DeviceScope->
				HardwareDeviceIdentifier != LastSeenGamepadHardwareDeviceIdentifier))
			{
				...
				const FName BestGamepadType = UCommonInputPlatformSettings::Get()->GetBestGamepadNameForHardware(
					GamepadInputType, DeviceScope->InputDeviceName, DeviceScope->HardwareDeviceIdentifier);
				if (BestGamepadType != GamepadInputType)
				{
					UE_LOG(LogCommonInput, Log,
					       TEXT("UCommonInputSubsystem: Autodetect changed GamepadInputType to %s"),
					       *BestGamepadType.ToString());
...
#if !UE_BUILD_SHIPPING
	if ((LocalPlayer.ViewportClient && LocalPlayer.ViewportClient->GetUseMouseForTouch()) || FSlateApplication::Get().
		IsFakingTouchEvents())
	{
		return ECommonInputType::Touch;
	}
