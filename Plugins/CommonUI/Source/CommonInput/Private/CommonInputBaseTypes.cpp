...
{
	const FCommonInputKeyBrushConfiguration* DisplayConfig = InputBrushDataMap.FindByPredicate(
		[&Key](const FCommonInputKeyBrushConfiguration& KeyBrushPair) -> bool
		{
...
bool UCommonInputBaseControllerData::TryGetInputBrush(FSlateBrush& OutBrush, const TArray<FKey>& Keys) const
{
	...
	const FCommonInputKeySetBrushConfiguration* DisplayConfig = InputBrushKeySets.FindByPredicate(
		[&Keys](const FCommonInputKeySetBrushConfiguration& KeyBrushPair) -> bool
		{
...
void UCommonInputBaseControllerData::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	if (!ObjectSaveContext.IsProceduralSave())
	{
		...
		InputBrushDataMap.Sort(
			[&AllKeys](const FCommonInputKeyBrushConfiguration& A, const FCommonInputKeyBrushConfiguration& B)
			{
...
const TArray<FName>& UCommonInputBaseControllerData::GetRegisteredGamepads()
{
	auto GenerateRegisteredGamepads = []()
	{
		...
		for (const TPair<FName, FDataDrivenPlatformInfo>& Platform :
		     FDataDrivenPlatformInfoRegistry::GetAllPlatformInfos())
		{
...
void UCommonInputBaseControllerData::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.ChangeType == EPropertyChangeType::ValueSet)
	{
		if (PropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(
			UCommonInputBaseControllerData, SetButtonImageHeightTo))
		{
...
bool UCommonInputPlatformSettings::TryGetInputBrush(FSlateBrush& OutBrush, FKey Key, ECommonInputType InputType,
                                                    const FName GamepadName) const
{
	InitializeControllerData();
	...
	for (const TSubclassOf<UCommonInputBaseControllerData>& ControllerDataPtr : ControllerDataClasses)
	{
		...
		{
			if (DefaultControllerData->InputType != ECommonInputType::Gamepad || DefaultControllerData->GamepadName ==
				GamepadName)
			{
...
bool UCommonInputPlatformSettings::TryGetInputBrush(FSlateBrush& OutBrush, const TArray<FKey>& Keys,
...
FName UCommonInputPlatformSettings::GetBestGamepadNameForHardware(FName CurrentGamepadName, FName InputDeviceName,
                                                                  const FString& HardwareDeviceIdentifier)
{
	InitializeControllerData();
	...
	if (ControllerDataClasses.Num() > 0)
	{
		for (const TSubclassOf<UCommonInputBaseControllerData>& ControllerDataPtr : ControllerDataClasses)
		{
			if (const UCommonInputBaseControllerData* DefaultControllerData = ControllerDataPtr.GetDefaultObject())
			{
				...
				{
					if ((Pair.InputDeviceName == InputDeviceName) && (Pair.HardwareDeviceIdentifier ==
...
bool FCommonInputPlatformBaseData::TryGetInputBrush(FSlateBrush& OutBrush, FKey Key, ECommonInputType InputType,
                                                    const FName& GamepadName) const
{
	if (ControllerDataClasses.Num() > 0)
	{
...
bool FCommonInputPlatformBaseData::TryGetInputBrush(FSlateBrush& OutBrush, const TArray<FKey>& Keys,
                                                    ECommonInputType InputType, const FName& GamepadName) const
{
	if (ControllerDataClasses.Num() > 0)
	{
