...
void UCommonInputSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	const UCommonInputPlatformSettings* Settings = UPlatformSettingsManager::Get().GetSettingsForPlatform<
	...
	TickHandle = FTSTicker::GetCoreTicker().AddTicker(
		FTickerDelegate::CreateUObject(this, &UCommonInputSubsystem::Tick), 0.1f);

	CVarInputKeysVisible->SetOnChangedCallback(
		FConsoleVariableDelegate::CreateUObject(this, &UCommonInputSubsystem::ShouldShowInputKeysChanged));
	...
	if (ICommonInputModule::Get().GetSettings().GetEnableEnhancedInputSupport())
	{
		if (ULocalPlayer* LocalPlayer = GetLocalPlayerChecked())
		{
			...
			{
				BroadcastInputMethodChangedEvent.BindUFunction(
					this, GET_FUNCTION_NAME_CHECKED(UCommonInputSubsystem, BroadcastInputMethodChanged));
				EnhancedInputLocalPlayerSubsystem->ControlMappingsRebuiltDelegate.AddUnique(
					BroadcastInputMethodChangedEvent);
			}
		}
	}
	...
	if (ICommonInputModule::Get().GetSettings().GetEnableEnhancedInputSupport())
	{
		if (ULocalPlayer* LocalPlayer = GetLocalPlayerChecked())
		{
			if (UEnhancedInputLocalPlayerSubsystem* EnhancedInputLocalPlayerSubsystem = LocalPlayer->GetSubsystem<
				UEnhancedInputLocalPlayerSubsystem>())
			{
				EnhancedInputLocalPlayerSubsystem->ControlMappingsRebuiltDelegate.Remove(
					BroadcastInputMethodChangedEvent);
...
void UCommonInputSubsystem::AddOrRemoveInputTypeLock(FName InReason, ECommonInputType InInputType, bool bAddLock)
{
	...
	if (bAddLock && PlatformSupportsInputType(InInputType))
	{
		...
		UE_LOG(LogCommonInput, Log, TEXT("Adding Input Method Lock: %s - %d"), *InReason.ToString(),
		       (int32)InInputType);
	}
	else
	{
		...
		UE_LOG(LogCommonInput, Log, TEXT("Removing Input Method Lock: %s - %d"), *InReason.ToString(),
		       (int32)InInputType);
	}
	...
	for (auto Entry : CurrentInputLocks)
	{
		...
		if (static_cast<int32>(Entry.Value) > ComputedInputLock)
		{
			ComputedInputLock = static_cast<int32>(Entry.Value);
		}
	}
	...
	{
		CurrentInputLock = static_cast<ECommonInputType>(ComputedInputLock);
	}
...
bool UCommonInputSubsystem::CheckForInputMethodThrashing(ECommonInputType NewInputType)
{
	...
	if (InputSettings.GetEnableInputMethodThrashingProtection())
	{
		...
		if (CurrentInputLocks.Contains(TEXT("InputMethodThrashing")))
		{
		...
		if (!CurrentInputLocks.Num())
		{
			...
			if (LockedInput != CurrentInputType)
			{
				...
				case ECommonInputType::Touch:
					...
					SlateApplication.SetGameIsFakingTouchEvents(
						LocalPlayer && LocalPlayer->ViewportClient && LocalPlayer->ViewportClient->
						GetUseMouseForTouch());
					break;
				case ECommonInputType::MouseAndKeyboard:
				default:
...
void UCommonInputSubsystem::UpdateCursorPosition(TSharedRef<FSlateUser> SlateUser, const FVector2D& NewPosition,
                                                 bool bForce)
{
...
void UCommonInputSubsystem::SetIsGamepadSimulatedClick(bool bNewIsGamepadSimulatedClick)
{
	if (bIsGamepadSimulatedClick != bNewIsGamepadSimulatedClick)
	{
		...
		UE_CLOG(bIsGamepadSimulatedClick, LogCommonInput, VeryVerbose,
		        TEXT("UCommonInputSubsystem::SetIsGamepadSimulatedClick(): Click is being simulated"));
	}
}
