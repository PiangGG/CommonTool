...
void UCommonVisibilitySwitcher::SetActiveWidgetIndex(int32 Index)
{
	...
	{
		UE_LOG(LogCommonUI, Verbose, TEXT("%s [%s] - [%d] is already the active index, doing nothing"),
		       ANSI_TO_TCHAR(__FUNCTION__), *GetName(), ActiveWidgetIndex);
		return;
	}
...
void UCommonVisibilitySwitcher::SetActiveWidget(const UWidget* Widget)
{
	...
	{
		UE_LOG(LogCommonUI, Verbose, TEXT("%s [%s] - [%s] is not a child of the switcher, doing nothing"),
		       ANSI_TO_TCHAR(__FUNCTION__), *GetName(), *Widget->GetName());
		return;
	}
...
void UCommonVisibilitySwitcher::ActivateVisibleSlot()
{
	...
	{
		UE_LOG(LogCommonUI, Verbose, TEXT("%s [%s] - Activating [%s]"), ANSI_TO_TCHAR(__FUNCTION__), *GetName(),
...
void UCommonVisibilitySwitcher::DeactivateVisibleSlot()
{
	if (UCommonActivatableWidget* ActivatableWidget = Cast<UCommonActivatableWidget>(GetActiveWidget()))
	{
		UE_LOG(LogCommonUI, Verbose, TEXT("%s [%s] - Deactivating [%s]"), ANSI_TO_TCHAR(__FUNCTION__), *GetName(),
		       *ActivatableWidget->GetName());
...
void UCommonVisibilitySwitcher::SetActiveWidgetIndex_Internal(int32 Index, bool bBroadcastChange /*= true*/)
{
	...
	{
		if (UCommonVisibilitySwitcherSlot* OldActiveSlot = Cast<
			UCommonVisibilitySwitcherSlot>(Slots[ActiveWidgetIndex]))
		{
			if (UCommonActivatableWidget* ActivatableWidget = Cast<UCommonActivatableWidget>(OldActiveSlot->Content))
			{
				UE_LOG(LogCommonUI, Verbose, TEXT("%s [%s] - Deactivating [%s]"), ANSI_TO_TCHAR(__FUNCTION__),
				       *GetName(), *ActivatableWidget->GetName());
				ActivatableWidget->DeactivateWidget();
			}

			UE_LOG(LogCommonUI, Verbose, TEXT("%s [%s] - Setting visibility of slot containing [%s] to Collapsed"),
			       ANSI_TO_TCHAR(__FUNCTION__), *GetName(), *OldActiveSlot->GetName());
	...
	if (Slots.IsValidIndex(ActiveWidgetIndex))
	{
		if (UCommonVisibilitySwitcherSlot* NewActiveSlot = Cast<
			UCommonVisibilitySwitcherSlot>(Slots[ActiveWidgetIndex]))
		{
			if (bAutoActivateSlot)
			{
				if (UCommonActivatableWidget* ActivatableWidget = Cast<
					UCommonActivatableWidget>(NewActiveSlot->Content))
				{
					UE_LOG(LogCommonUI, Verbose, TEXT("%s [%s] - Activating [%s]"), ANSI_TO_TCHAR(__FUNCTION__),
					       *GetName(), *ActivatableWidget->GetName());
					ActivatableWidget->ActivateWidget();
				}
			}

			UE_LOG(LogCommonUI, Verbose, TEXT("%s [%s] - Setting visibility of slot containing [%s] to [%s]"),
			       ANSI_TO_TCHAR(__FUNCTION__), *GetName(), *NewActiveSlot->GetName(),
			       *StaticEnum<ESlateVisibility>()->GetDisplayValueAsText(ShownVisibility).ToString());
...
{
	if (UWidget* SelectedChild = FindChildContainingDescendant(this, DescendantWidget))
	{
		...
		if (UCommonActivatableWidget* ActivatableWidget = Cast<UCommonActivatableWidget>(Child))
		{
			if (ActivatableWidget->SetsVisibilityOnActivated())
			{
				CompileLog.Error(FText::Format(
					FText::FromString(
						"{0} has bSetVisibilityOnActivated enabled but is in a switcher. Its visibility will be set by the switcher."),
					FText::FromString(ActivatableWidget->GetName())));
			}
			...
			if (ActivatableWidget->SetsVisibilityOnDeactivated())
			{
				CompileLog.Error(FText::Format(
					FText::FromString(
						"{0} has bSetVisibilityOnDeactivated enabled but is in a switcher. Its visibility will be set by the switcher."),
					FText::FromString(ActivatableWidget->GetName())));
			}
		}
	}
