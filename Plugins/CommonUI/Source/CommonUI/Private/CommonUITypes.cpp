...
const FCommonInputTypeInfo& FCommonInputActionDataBase::GetCurrentInputTypeInfo(
	const UCommonInputSubsystem* CommonInputSubsystem) const
{
	...
	// may not always have a valid input Subsystem, e.g. for the demo driver's SpectatorPlayerController when recording a replay
	{
...
const FCommonInputTypeInfo& FCommonInputActionDataBase::GetInputTypeInfo(
	ECommonInputType InputType, const FName& GamepadName) const
{
	if (InputType == ECommonInputType::MouseAndKeyboard)
	{
	...
	if (InputType == ECommonInputType::Gamepad)
	{
		...
		UE_CLOG(!GamepadTypeInfo->GetKey().IsValid(), LogCommonUI, Verbose,
		        TEXT("Invalid default common action key for action \"%s\""), *DisplayName.ToString());
...
bool FCommonInputActionDataBase::IsKeyBoundToInputActionData(const FKey& Key,
                                                             const UCommonInputSubsystem* CommonInputSubsystem) const
{
...
FSlateBrush FCommonInputActionDataBase::GetCurrentInputActionIcon(
	const UCommonInputSubsystem* CommonInputSubsystem) const
{
	...
	if (UCommonInputPlatformSettings::Get()->TryGetInputBrush(SlateBrush, CurrentInputTypeInfo.GetKey(),
...
void FCommonInputActionDataBase::OnPostDataImport(const UDataTable* InDataTable, const FName InRowName,
...
void FCommonInputActionDataBase::AddGamepadInputOverride(const FName& GamepadName,
                                                         const FCommonInputTypeInfo& InputInfo)
{
	GamepadInputOverrides.Add(GamepadName, InputInfo);
}

const UCommonInputMetadata* UCommonMappingContextMetadata::GetCommonInputMetadata(
	const UInputAction* InInputAction) const
{
	if (const TObjectPtr<const UCommonInputMetadata>* PerActionMetdata = PerActionEnhancedInputMetadata.Find(
		InInputAction))
	{
...
{
	return InputActionRowHandle.GetRow<FCommonInputActionDataBase>(TEXT(
		"CommonUIUtils::GetInputActionData couldn't find the row passed in, check data table if its missing."));
}

FSlateBrush CommonUI::GetIconForInputActions(const UCommonInputSubsystem* CommonInputSubsystem,
                                             const TArray<FDataTableRowHandle>& InputActions)
{
	...
	FSlateBrush SlateBrush;
	if (UCommonInputPlatformSettings::Get()->TryGetInputBrush(SlateBrush, Keys,
	                                                          CommonInputSubsystem->GetCurrentInputType(),
	                                                          CommonInputSubsystem->GetCurrentGamepadName()))
	{
		return SlateBrush;
...
TObjectPtr<const UCommonInputMetadata> CommonUI::GetEnhancedInputActionMetadata(const UInputAction* InputAction)
{
	...
	if (ICommonMappingContextMetadataInterface* MappingContextMetadata = Cast<ICommonMappingContextMetadataInterface>(
		InputAction->GetPlayerMappableKeySettings()->Metadata))
	{
...
void CommonUI::GetEnhancedInputActionKeys(const ULocalPlayer* LocalPlayer, const UInputAction* InputAction,
                                          TArray<FKey>& OutKeys)
{
	if (LocalPlayer)
	{
		if (UEnhancedInputLocalPlayerSubsystem* EnhancedInputLocalPlayerSubsystem = LocalPlayer->GetSubsystem<
...
void CommonUI::InjectEnhancedInputForAction(const ULocalPlayer* LocalPlayer, const UInputAction* InputAction,
...
FSlateBrush CommonUI::GetIconForEnhancedInputAction(const UCommonInputSubsystem* CommonInputSubsystem,
                                                    const UInputAction* InputAction)
{
	FKey FirstKeyForCurrentInput = GetFirstKeyForInputType(CommonInputSubsystem->GetLocalPlayer(),
	                                                       CommonInputSubsystem->GetCurrentInputType(), InputAction);

	FSlateBrush SlateBrush;
	if (FirstKeyForCurrentInput.IsValid() && UCommonInputPlatformSettings::Get()->TryGetInputBrush(
		SlateBrush, FirstKeyForCurrentInput, CommonInputSubsystem->GetCurrentInputType(),
		CommonInputSubsystem->GetCurrentGamepadName()))
	{
		return SlateBrush;
	}
...
bool CommonUI::ActionValidForInputType(const ULocalPlayer* LocalPlayer, ECommonInputType InputType,
                                       const UInputAction* InputAction)
{
	if (!LocalPlayer)
	{
	...
	for (const FKey& Key : Keys)
	{
		...
		bool bIsValidMouseAndKeyboard = !Key.IsTouch() && !Key.IsGamepadKey() && InputType ==
...
FKey CommonUI::GetFirstKeyForInputType(const ULocalPlayer* LocalPlayer, ECommonInputType InputType,
                                       const UInputAction* InputAction)
{
	if (!LocalPlayer)
	{
