...
void UCommonNumericTextBlock::Tick(float DeltaSeconds)
{
	...
	const float CurrentStateOverage = FMath::Max(
		0.0f, InterpolationState.ElapsedStateDuration + DeltaSeconds - CurrentStateDuration);
	InterpolationState.ElapsedStateDuration = FMath::Min(InterpolationState.ElapsedStateDuration + DeltaSeconds,
...
void UCommonNumericTextBlock::OnTimerTick()
{
	...
	static constexpr float MinimumUpdateInterval = 0.05f;
...
void UCommonNumericTextBlock::InterpolateToValue(const float InTargetValue,
                                                 float InMaximumInterpolationDuration /*= 3.0f*/,
                                                 float InMinimumChangeRate /*= 1.0f*/, float InOutroOffset /*= 0.0f*/)
{
	...
	const float InterpolationDuration = CalculateInterpolationDuration(InTargetValue, InMinimumChangeRate,
	                                                                   InMaximumInterpolationDuration);
...
float UCommonNumericTextBlock::CalculateInterpolationDuration(const float InTargetValue,
                                                              const float InMinimumChangeRate,
                                                              const float InMaximumInterpolationDuration) const
{
	...
	if (FMath::Abs(SignedMinimumChangeRate) < FMath::Abs(InTargetValue - CurrentNumericValue))
	{
		...
		const float DurationBasedOnMinimumChangeRate = 1.0 / (-FMath::Pow(
			-SignedMinimumChangeRate / (InTargetValue - CurrentNumericValue) + 1.0,
			1.0 / EaseOutInterpolationExponent) + 1.0);
	...
	return FMath::Clamp((InTargetValue - CurrentNumericValue) / SignedMinimumChangeRate, 0.0f,
	                    InMaximumInterpolationDuration);
}
...
FText UCommonNumericTextBlock::FormatText(float InCurrentNumericValue,
                                          const FNumberFormattingOptions& InNumberFormattingOptions) const
{
	switch (NumericType)
	{
	...
	case ECommonNumericType::Distance:
		{
			FNumericUnit<float> DisplayUnit = FUnitConversion::QuantizeUnitsToBestFit<float>(
				InCurrentNumericValue, EUnit::Centimeters);
...
void UCommonNumericTextBlock::EnterNumericInterpolation(const float InitialValue, const float FinalValue,
                                                        const float Duration, const float OutroOffset)
{
...
void UCommonNumericTextBlock::UpdateNumericInterpolation()
{
	...
	const float Alpha = FMath::Clamp(InterpolationState.ElapsedStateDuration / NumericInterpolationState.Duration, 0.0f,
	                                 1.0f);
	CurrentNumericValue = FMath::InterpEaseOut(NumericInterpolationState.SourceValue,
	                                           NumericInterpolationState.DestinationValue, Alpha,
	...
	const bool ShouldHaveTriggeredOutro = InterpolationState.ElapsedStateDuration >= NumericInterpolationState.Duration
		- NumericInterpolationState.OutroOffset;
	...
	const bool HasNumericInterpolationDurationElapsed = InterpolationState.ElapsedStateDuration >=
		NumericInterpolationState.Duration;
...
void UCommonNumericTextBlock::UpdateSizeInterpolation()
{
	...
	const int32 TargetDesiredWidth = UnderlyingWidget->ComputeDesiredSize(
		FSlateApplication::Get().GetApplicationScale()).X;

	// Set text block's minimum desired width based on "ease out" interpolation.
	const float Alpha = FMath::Clamp(InterpolationState.ElapsedStateDuration / SizeInterpolationState.Duration, 0.0f,
	                                 1.0f);
	SetMinDesiredWidth(FMath::InterpEaseOut(SizeInterpolationState.InitialWidth, TargetDesiredWidth, Alpha,
	                                        EaseOutInterpolationExponent));

	// If the shrink duration has elapsed...
	const bool HasSizeInterpolationDurationElapsed = InterpolationState.ElapsedStateDuration >= SizeInterpolationState.
		Duration;
