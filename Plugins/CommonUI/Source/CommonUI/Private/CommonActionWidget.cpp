...
TSharedRef<SWidget> UCommonActionWidget::RebuildWidget()
{
	...
	{
		UMaterialInstanceDynamic* const ParentMaterialDynamic = Cast<UMaterialInstanceDynamic>(
			ProgressMaterialBrush.GetResourceObject());
...
FSlateBrush UCommonActionWidget::GetIcon() const
{
	...
	else
	{
		if (DesignTimeKey.IsValid())
		{
			...
			if (UCommonInputPlatformSettings::Get()->TryGetInputBrush(InputBrush, TArray<FKey>{DesignTimeKey},
			                                                          KeyInputType, OutDefaultGamepadName))
			{
				return InputBrush;
			}
		}
...
void UCommonActionWidget::UpdateActionWidget()
{
	...
	{
		const UCommonInputSubsystem* CommonInputSubsystem = GetInputSubsystem();
		if (IsDesignTime() || (GetGameInstance() && ensure(CommonInputSubsystem) && CommonInputSubsystem->
			ShouldShowInputKeys()))
		{
...
void UCommonActionWidget::SetProgressMaterial(const FSlateBrush& InProgressMaterialBrush,
