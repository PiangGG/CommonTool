...
void SAnalogSlider::Construct(const FArguments& InDeclaration)
{
	SetCanTick(false);
...
FReply SAnalogSlider::OnAnalogValueChanged(const FGeometry& MyGeometry, const FAnalogInputEvent& InAnalogInputEvent)
{
	...
	constexpr float SlowestStepTime = 1.0f;
	constexpr float TimeToSlideFromZeroToOneAtFullSpeed = 3;
	const float FastestStepTime = TimeToSlideFromZeroToOneAtFullSpeed * StepSizeValue;

	float NewValue = GetValue();
	bool bCommitNewValue = false;

	if (Orientation == Orient_Horizontal && KeyPressed == EKeys::Gamepad_LeftX)
	{
		Reply = FReply::Handled();
	}
	else if (Orientation == Orient_Vertical && KeyPressed == EKeys::Gamepad_LeftY)
	{
		Reply = FReply::Handled();
	}
	else
	...
	if (bIsUsingGamepad)
	{
		...
		{
			const float NormalizedStepSpeed = FMath::GetMappedRangeValueClamped(
				FVector2f(AnalogStepThreshold, 1), FVector2f(0, 1), AbsAnalogValue);
			...
			if (TimeSinceLastStep > RepeatTime)
			{
				...
				{
					StepMultiplier = FMath::Max(1, static_cast<int32>(FApp::GetDeltaTime() / RepeatTime));
				}
