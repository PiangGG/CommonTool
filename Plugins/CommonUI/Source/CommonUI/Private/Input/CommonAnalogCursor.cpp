...
constexpr float AnalogScrollUpdatePeriod = 0.1f;
...
void FCommonAnalogCursor::Tick(const float DeltaTime, FSlateApplication& SlateApp, TSharedRef<ICursor> Cursor)
{
	...
	if (IsUsingGamepad() && IsGameViewportInFocusPathWithoutCapture())
	{
		...
		else
		{
			...
			if (TSharedPtr<ITableViewMetadata> TableViewMetadata = CursorTarget
				                                                       ? CursorTarget->GetMetaData<ITableViewMetadata>()
				                                                       : nullptr)
			{
			...
			if (CursorTarget != PinnedLastCursorTarget || (CursorTarget && CursorTarget->GetCachedGeometry().
				GetAccumulatedRenderTransform() != LastCursorTargetTransform))
			{
#if !UE_BUILD_SHIPPING
				if (CursorTarget != PinnedLastCursorTarget)
				{
					UE_LOG(LogCommonUI, Verbose, TEXT("User[%d] cursor target changed to [%s]"), GetOwnerUserIndex(),
					       *FReflectionMetaData::GetWidgetDebugInfo(CursorTarget.Get()));
				}
#endif

				// Release capture unless the focused widget is the captor
				if (PinnedLastCursorTarget != CursorTarget && SlateUser->HasCursorCapture() && !SlateUser->
					DoesWidgetHaveAnyCapture(CursorTarget))
				{
					UE_LOG(LogCommonUI, Log,
					       TEXT(
						       "User[%d] focus changed while the cursor is captured - releasing now before moving cursor to focused widget."
					       ), GetOwnerUserIndex());
				...
				if (CursorTarget)
				{
					...
					if (CursorTarget == GetViewportClient()->GetGameViewportWidget())
					{
						...
						{
							TargetGeometry = GameLayerManager->GetPlayerWidgetHostGeometry(
								ActionRouter.GetLocalPlayerChecked());
						}
					}
					else
					{
					...
					if (TargetGeometry.GetLocalSize().X > UE_SMALL_NUMBER && TargetGeometry.GetLocalSize().Y >
						UE_SMALL_NUMBER)
					{
						...
						UE_LOG(LogCommonUI, Verbose, TEXT("User[%d] moving cursor to target [%s] @ (%d, %d)"),
						       GetOwnerUserIndex(), *FReflectionMetaData::GetWidgetDebugInfo(CursorTarget.Get()),
						       (int32)AbsoluteWidgetCenter.X, (int32)AbsoluteWidgetCenter.Y);
					}
				}
		...
		if (bShouldHandleRightAnalog)
		{
			...
			if (TimeUntilScrollUpdate <= 0.0f && GetAnalogValues(EAnalogStick::Right).SizeSquared() > FMath::Square(
				ScrollDeadZone))
			{
				// Generate mouse wheel events over all widgets currently registered as scroll recipients
				const TArray<const UWidget*>& AnalogScrollRecipients = ActionRouter.
					GatherActiveAnalogScrollRecipients();
				if (AnalogScrollRecipients.Num() > 0)
				{
					const FCommonAnalogCursorSettings& CursorSettings = UCommonUIInputSettings::Get().
						GetAnalogCursorSettings();
					const auto GetScrollAmountFunc = [&CursorSettings](float AnalogValue)
					{
						...
						return (AmountBeyondDeadZone / (1.f - CursorSettings.ScrollDeadZone)) * -
							FMath::Sign(AnalogValue) * CursorSettings.ScrollMultiplier;
					};

					const FVector2D& RightStickValues = GetAnalogValues(EAnalogStick::Right);
					const FVector2D ScrollAmounts(GetScrollAmountFunc(RightStickValues.X),
					                              GetScrollAmountFunc(RightStickValues.Y));
					...
					for (const UWidget* ScrollRecipient : AnalogScrollRecipients)
					{
						...
						if (ScrollRecipient->GetCachedWidget())
						{
							...
							const float ScrollAmount = Orientation == Orient_Vertical
								                           ? ScrollAmounts.Y
								                           : ScrollAmounts.X;
							if (FMath::Abs(ScrollAmount) > SMALL_NUMBER)
							{
								const FVector2D WidgetCenter = ScrollRecipient->GetCachedGeometry().
									GetAbsolutePositionAtCoordinates(FVector2D(.5f, .5f));
...
bool FCommonAnalogCursor::HandleKeyDownEvent(FSlateApplication& SlateApp, const FKeyEvent& InKeyEvent)
{
	if (IsRelevantInput(InKeyEvent))
	{
		...
		if (LocalPlayer.ViewportClient && LocalPlayer.ViewportClient->ViewportConsole && LocalPlayer.ViewportClient->
			ViewportConsole->ConsoleActive())
		{
		...
		if (PressedKey == EKeys::Gamepad_LeftShoulder) { ShoulderButtonStatus |= LeftShoulder; }
		if (PressedKey == EKeys::Gamepad_RightShoulder) { ShoulderButtonStatus |= RightShoulder; }
		if (PressedKey == EKeys::Gamepad_LeftTrigger) { ShoulderButtonStatus |= LeftTrigger; }
		if (PressedKey == EKeys::Gamepad_RightTrigger) { ShoulderButtonStatus |= RightTrigger; }

		if (ShoulderButtonStatus == All)
		{
			ShoulderButtonStatus = None;
		...
		if (bIsVirtualAccept && ActionRouter.ProcessInput(InKeyEvent.GetKey(), InputEventType) ==
			ERouteUIInputResult::Handled)
		{
			return true;
		}
		if (!bIsVirtualAccept || ShouldVirtualAcceptSimulateMouseButton(InKeyEvent, IE_Pressed))
		{
...
bool FCommonAnalogCursor::HandleKeyUpEvent(FSlateApplication& SlateApp, const FKeyEvent& InKeyEvent)
{
	if (IsRelevantInput(InKeyEvent))
	{
		...
		// We support binding actions to the virtual accept key, so it's a special flower that gets processed right now
		const bool bIsVirtualAccept = InKeyEvent.GetKey() == EKeys::Virtual_Accept;
		if (bIsVirtualAccept && ActionRouter.ProcessInput(InKeyEvent.GetKey(), IE_Released) ==
			ERouteUIInputResult::Handled)
		{
			return true;
		}
		if (!bIsVirtualAccept || ShouldVirtualAcceptSimulateMouseButton(InKeyEvent, IE_Released))
		{
...
bool FCommonAnalogCursor::CanReleaseMouseCapture() const
{
	...
	return MouseCapture == EMouseCaptureMode::CaptureDuringMouseDown || MouseCapture ==
		EMouseCaptureMode::CapturePermanently_IncludingInitialMouseDown;
}

bool FCommonAnalogCursor::HandleAnalogInputEvent(FSlateApplication& SlateApp,
			...
			&& EffectingButton != EKeys::MiddleMouseButton)
		{
			...
			if (TSharedPtr<SWidget> ViewportWidget = ViewportClient ? ViewportClient->GetGameViewportWidget() : nullptr)
			{
				const FWidgetPath WidgetsUnderCursor = SlateApp.LocateWindowUnderMouse(
					PointerEvent.GetScreenSpacePosition(), SlateApp.GetInteractiveTopLevelWindows());
				if (WidgetsUnderCursor.ContainsWidget(ViewportWidget.Get()))
				{
					FKeyEvent MouseKeyEvent(EffectingButton, PointerEvent.GetModifierKeys(),
					                        PointerEvent.GetUserIndex(), false, 0, 0);
...
bool FCommonAnalogCursor::HandleMouseButtonUpEvent(FSlateApplication& SlateApp, const FPointerEvent& PointerEvent)
{
	if (FAnalogCursor::IsRelevantInput(PointerEvent))
	{
		...
		if (bHadKeyDown
			|| (IsEligibleFakeKeyPointerEvent(PointerEvent) && !SlateApp.HasUserMouseCapture(
				PointerEvent.GetUserIndex())))
		{
			// Reprocess as a key if there was no mouse capture or it was previously pressed
			FKeyEvent MouseKeyEvent(PointerEvent.GetEffectingButton(), PointerEvent.GetModifierKeys(),
			                        PointerEvent.GetUserIndex(), false, 0, 0);
...
EOrientation FCommonAnalogCursor::DetermineScrollOrientation(const UWidget& Widget) const
{
	...
	if (const UScrollBar* AsScrollBar = Cast<const UScrollBar>(&Widget))
	{
		return AsScrollBar->GetOrientation();
	}
	if (const UScrollBox* AsScrollBox = Cast<const UScrollBox>(&Widget))
	{
...
{
	return IsUsingGamepad() && FAnalogCursor::IsRelevantInput(KeyEvent) && (IsGameViewportInFocusPathWithoutCapture() ||
		(KeyEvent.GetKey() == EKeys::Virtual_Accept && CanReleaseMouseCapture()));
}

bool FCommonAnalogCursor::IsRelevantInput(const FAnalogInputEvent& AnalogInputEvent) const
{
	return IsUsingGamepad() && FAnalogCursor::IsRelevantInput(AnalogInputEvent) &&
...
void FCommonAnalogCursor::RefreshCursorVisibility()
{
	...
	{
		const bool bShowCursor = bIsAnalogMovementEnabled || ActionRouter.ShouldAlwaysShowCursor() || ActiveInputMethod
...
void FCommonAnalogCursor::HideCursor()
{
	...
	if (SlateUser && World && World->IsGameWorld())
	{
		...
		{
			const FVector2D TopLeftPos = GameViewport->Viewport->ViewportToVirtualDesktopPixel(
				FVector2D(0.025f, 0.025f));
...
void FCommonAnalogCursor::SetNormalizedCursorPosition(const FVector2D& RelativeNewPosition)
{
	...
	if (SlateUser)
	{
		...
		{
			const FVector2D ClampedNewPosition(FMath::Clamp(RelativeNewPosition.X, 0.0f, 1.0f),
			                                   FMath::Clamp(RelativeNewPosition.Y, 0.0f, 1.0f));
			const FVector2D AbsolutePosition = ViewportWidget->GetCachedGeometry().GetAbsolutePositionAtCoordinates(
				ClampedNewPosition);
