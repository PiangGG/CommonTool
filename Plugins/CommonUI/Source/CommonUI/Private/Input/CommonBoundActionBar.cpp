...
void UCommonBoundActionBar::OnWidgetRebuilt()
{
	...
	if (const UGameInstance* GameInstance = GetGameInstance())
	{
		...
		{
			GameInstance->GetGameViewportClient()->OnPlayerAdded().AddUObject(
				this, &UCommonBoundActionBar::HandlePlayerAdded);
		}
...
void UCommonBoundActionBar::ValidateCompiledDefaults(IWidgetCompilerLog& CompileLog) const
{
	...
	if (!ActionButtonClass)
	{
		CompileLog.Error(FText::Format(
			LOCTEXT("Error_BoundActionBar_MissingButtonClass", "{0} has no ActionButtonClass specified."),
			FText::FromString(GetName())));
	}
	else if (CompileLog.GetContextClass() && ActionButtonClass->IsChildOf(CompileLog.GetContextClass()))
	{
		CompileLog.Error(FText::Format(LOCTEXT("Error_BoundActionBar_RecursiveButtonClass",
		                                       "{0} has a recursive ActionButtonClass specified (reference itself)."), FText::FromString(
			                               GetName())));
	}
}
#endif
...
void UCommonBoundActionBar::HandleDeferredDisplayUpdate()
{
	...
	for (const ULocalPlayer* LocalPlayer : SortedPlayers)
	{
		if (LocalPlayer == OwningLocalPlayer || !bDisplayOwningPlayerActionsOnly)
		{
			...
			{
				if (const UCommonUIActionRouterBase* ActionRouter = ULocalPlayer::GetSubsystem<
					UCommonUIActionRouterBase>(LocalPlayer))
				{
					...
					TArray<FUIActionBindingHandle> FilteredBindings = ActionRouter->GatherActiveBindings().
						FilterByPredicate(
							[ActionRouter, PlayerInputType, PlayerGamepadName, &AcceptedBindings, this](
							const FUIActionBindingHandle& Handle) mutable
							{
								if (TSharedPtr<FUIActionBinding> Binding = FUIActionBinding::FindBinding(Handle))
								{
									...
									if (CommonUI::IsEnhancedInputSupportEnabled())
									{
										...
										{
											if (CommonUI::ActionValidForInputType(
												ActionRouter->GetLocalPlayer(), PlayerInputType, InputAction))
											{
					...
					Algo::Sort(FilteredBindings,
					           [ActionRouter, PlayerInputType, PlayerGamepadName](
					           const FUIActionBindingHandle& A, const FUIActionBindingHandle& B)
					           {
						           ...
						           if (ensureMsgf((BindingA && BindingB),
						                          TEXT(
							                          "The array filter above should enforce that there are no null bindings"
						                          )))
						           {
							           auto IsKeyBackAction = [ActionRouter, PlayerInputType, PlayerGamepadName](
								           FCommonInputActionDataBase* LegacyData, const UInputAction* InputAction)
							           {
								           if (LegacyData)
								           {
									           FKey Key = LegacyData->GetInputTypeInfo(
										           PlayerInputType, PlayerGamepadName).GetKey();
									           ...
									           if (PlayerInputType == ECommonInputType::Touch)
									           {
										           if (!Key.IsValid())
										           {
											           Key = LegacyData->GetInputTypeInfo(
												           ECommonInputType::MouseAndKeyboard,
												           PlayerGamepadName).GetKey();
										           }
									           }

									           return Key == EKeys::Virtual_Back || Key == EKeys::Escape || Key ==
										           EKeys::Android_Back;
								           }
								           if (InputAction)
								           {
									           FKey Key = CommonUI::GetFirstKeyForInputType(
										           ActionRouter->GetLocalPlayer(), PlayerInputType, InputAction);
									           ...
									           if (PlayerInputType == ECommonInputType::Touch)
									           {
										           if (!Key.IsValid())
										           {
											           Key = CommonUI::GetFirstKeyForInputType(
												           ActionRouter->GetLocalPlayer(),
												           ECommonInputType::MouseAndKeyboard, InputAction);
										           }
									           }
							           ...
							           auto GetNavBarPriority = [](FCommonInputActionDataBase* LegacyData,
							                                       const UInputAction* InputAction)
							           {
								           ...
								           if (InputAction)
								           {
									           if (TObjectPtr<const UCommonInputMetadata> InputActionMetadata =
										           CommonUI::GetEnhancedInputActionMetadata(InputAction))
									           {
							           ...
							           if (ensureMsgf((bIsValidActionA && bIsValidActionB),
							                          TEXT(
								                          "Action bindings not displayed yet -- array filter enforces they are not included"
							                          )))
							           {
					...
					{
						ICommonBoundActionButtonInterface* ActionButton = Cast<ICommonBoundActionButtonInterface>(
							CreateActionButton(BindingHandle));
...
{
	if (const UCommonUIActionRouterBase* ActionRouter = ULocalPlayer::GetSubsystem<
		UCommonUIActionRouterBase>(NewPlayer))
	{
		ActionRouter->OnBoundActionsUpdated().AddUObject(this, &UCommonBoundActionBar::HandleBoundActionsUpdated,
		                                                 NewPlayer == GetOwningLocalPlayer());
	}
