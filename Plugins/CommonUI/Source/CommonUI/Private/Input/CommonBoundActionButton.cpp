...
void UCommonBoundActionButton::UpdateInputActionWidget()
{
	if (InputActionWidget) //optional bound widget
	{
		...
		if (BindingHandle.IsValid())
		{
			...
			{
				TOptional<int32> BoundPlayerIndex = FSlateApplication::Get().GetUserIndexForController(
					BindingOwner->GetControllerId());
				if (BoundPlayerIndex.IsSet())
				{
					...
					ActionDisplayName = FText::FormatNamed(
						LOCTEXT("OtherPlayerActionFormat", "[P{PlayerNum}] {ActionName}"),
