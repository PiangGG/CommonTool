	  ...
	  , InputAction(BindArgs.InputAction)
{
	...
	else
	{
		...
		FName CurrentGamepadName = UCommonUIActionRouterBase::Get(InBoundWidget)->GetInputSubsystem().
			GetCurrentGamepadName();

		// KB/M
		const FCommonInputTypeInfo& LegacyMapping_KBM = LegacyActionData->GetInputTypeInfo(
			ECommonInputType::MouseAndKeyboard, FCommonInputDefaults::GamepadGeneric);
		FUIActionKeyMapping KeyMapping_KBM(LegacyMapping_KBM.GetKey(),
		                                   LegacyMapping_KBM.bActionRequiresHold ? LegacyMapping_KBM.HoldTime : 0.f,
		                                   LegacyMapping_KBM.bActionRequiresHold
			                                   ? LegacyMapping_KBM.HoldRollbackTime
			                                   : 0.f);

		// Touch
		const FCommonInputTypeInfo& LegacyMapping_Touch = LegacyActionData->GetInputTypeInfo(
			ECommonInputType::Touch, FCommonInputDefaults::GamepadGeneric);
		FUIActionKeyMapping KeyMapping_Touch(LegacyMapping_Touch.GetKey(),
		                                     LegacyMapping_Touch.bActionRequiresHold
			                                     ? LegacyMapping_Touch.HoldTime
			                                     : 0.f, LegacyMapping_Touch.bActionRequiresHold
				                                            ? LegacyMapping_Touch.HoldRollbackTime
				                                            : 0.f);
		...
		check(ActionRouter);
		const FCommonInputTypeInfo& LegacyMapping_Gamepad = LegacyActionData->GetInputTypeInfo(
			ECommonInputType::Gamepad, ActionRouter->GetInputSubsystem().GetCurrentGamepadName());
		FUIActionKeyMapping KeyMapping_Gamepad(LegacyMapping_Gamepad.GetKey(),
		                                       LegacyMapping_Gamepad.bActionRequiresHold
			                                       ? LegacyMapping_Gamepad.HoldTime
			                                       : 0.f, LegacyMapping_Gamepad.bActionRequiresHold
				                                              ? LegacyMapping_Gamepad.HoldRollbackTime
				                                              : 0.f);

		if (BindArgs.bForceHold && ICommonInputModule::GetSettings().GetDefaultHoldData())
		{
			UCommonUIHoldData* CommonUIHoldBehaviorValues = ICommonInputModule::GetSettings().GetDefaultHoldData().
				GetDefaultObject();

			KeyMapping_KBM.HoldTime = KeyMapping_KBM.HoldTime > 0.f
				                          ? KeyMapping_KBM.HoldTime
				                          : CommonUIHoldBehaviorValues->KeyboardAndMouse.HoldTime;
			KeyMapping_Touch.HoldTime = KeyMapping_Touch.HoldTime > 0.f
				                            ? KeyMapping_Touch.HoldTime
				                            : CommonUIHoldBehaviorValues->Touch.HoldTime;
			KeyMapping_Gamepad.HoldTime = KeyMapping_Gamepad.HoldTime > 0.f
				                              ? KeyMapping_Gamepad.HoldTime
				                              : CommonUIHoldBehaviorValues->Gamepad.HoldTime;
		}
...
FUIActionBindingHandle FUIActionBinding::TryCreate(const UWidget& InBoundWidget, const FBindUIActionArgs& BindArgs)
{
	...
	{
		UE_LOG(LogUIActionRouter, Error, TEXT("Cannot create action binding for widget [%s] - no action provided."),
		       *InBoundWidget.GetName());
		return FUIActionBindingHandle();
	}
	if (!BindArgs.OnExecuteAction.IsBound())
	{
		UE_LOG(LogUIActionRouter, Error, TEXT("Cannot bind widget [%s] to action [%s] - empty handler delegate."),
		       *InBoundWidget.GetName(), *BindArgs.GetActionName().ToString());
		return FUIActionBindingHandle();
	}
	if (BindArgs.ActionTag.IsValid() && !UCommonUIInputSettings::Get().FindAction(BindArgs.ActionTag))
	{
		UE_LOG(LogUIActionRouter, Error,
		       TEXT(
			       "Cannot bind widget [%s] to action [%s] - provided tag does not map to an existing UI input action. It can be added under Project Settings->UI Input."
		       ), *InBoundWidget.GetName(), *BindArgs.GetActionName().ToString());
		return FUIActionBindingHandle();
	}
	if (!BindArgs.LegacyActionTableRow.IsNull() && !BindArgs.LegacyActionTableRow.GetRow<
		FCommonInputActionDataBase>(TEXT("")))
	{
		UE_LOG(LogUIActionRouter, Error,
		       TEXT(
			       "Cannot bind widget [%s] to action [%s] - provided legacy data table row does not resolve to valid data."
		       ), *InBoundWidget.GetName(), *BindArgs.GetActionName().ToString());
		return FUIActionBindingHandle();
	}
	if (!BindArgs.ActionTag.IsValid() && BindArgs.LegacyActionTableRow.IsNull() && bIsEnhancedInputSupportEnabled && !
		BindArgs.InputAction.IsValid())
	{
		UE_LOG(LogUIActionRouter, Error,
		       TEXT("Cannot bind widget [%s] to action [%s] - provided input action is invalid."),
		       *InBoundWidget.GetName(), *BindArgs.GetActionName().ToString());
		return FUIActionBindingHandle();
	}

	// Make sure there is no existing binding for the same action associated with the same widget
	for (const TPair<FUIActionBindingHandle, TSharedPtr<FUIActionBinding>>& HandleBindingPair :
	     AllRegistrationsByHandle)
	{
			...
			HandleBindingPair.Value->BoundWidget.Get() == &InBoundWidget)
		{
			UE_LOG(LogUIActionRouter, Error,
			       TEXT(
				       "Widget [%s] is already bound to action [%s] [%s]! A given widget can only bind the same action once. Unregister existing binding first if you wish to change any aspect of the binding"
			       ),
			       *InBoundWidget.GetName(), *HandleBindingPair.Value->ActionName.ToString(),
			       InputEventToString(BindArgs.KeyEvent));
			return FUIActionBindingHandle();
		}
	}
...
void FUIActionBinding::CleanRegistrations()
{
	...
	{
		if (!Iter->Value->BoundWidget.IsValid() || !ensureMsgf(Iter->Value->OnExecuteAction.IsBound(),
		                                                       TEXT("Unbound Action: %s"),
		                                                       *Iter->Value->ActionName.ToString()))
		{
...
bool FUIActionBinding::UpdateHold(float TargetHoldTime)
{
	...
	UE_LOG(LogUIActionRouter, VeryVerbose, TEXT("Hold repeating: %s for %f"), *ActionName.ToString(),
	       CurrentHoldSecond);
	...
	if (HeldPercent >= 1.f)
	{
		...
		UE_LOG(LogUIActionRouter, VeryVerbose, TEXT("Hold repeating: Fired completed action! %s after %f"),
		       *ActionName.ToString(), CurrentHoldSecond);
		return true;
	}
	return false;
}
...
void FUIActionBinding::BeginRollback(float TargetHoldRollbackTime, float InHoldTime,
                                     FUIActionBindingHandle BindingHandle)
{
	HoldRollbackMultiplier = InHoldTime / TargetHoldRollbackTime;
	HoldTime = InHoldTime;

	HoldProgressRollbackTickerHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda(
		[BindingHandle](float DeltaTime)
		{
			if (TSharedPtr<FUIActionBinding> InBinding = FindBinding(BindingHandle))
			{
				...
				InBinding->CurrentHoldSecond = FMath::Clamp(
					InBinding->CurrentHoldSecond - (DeltaTime * InBinding->HoldRollbackMultiplier), 0.f,
					HoldRollbackTime);
				const float HoldRollbackPercent = FMath::Clamp(InBinding->CurrentHoldSecond / InBinding->HoldTime, 0.f,
				                                               1.f);
...
bool FBindUIActionArgs::ActionHasHoldMappings() const
{
	...
	{
		if (FCommonInputActionDataBase* InputActionData = LegacyActionTableRow.GetRow<
			FCommonInputActionDataBase>(TEXT("")))
		{
...
FText FUIActionBindingHandle::GetDisplayName() const
{
	if (TSharedPtr<const FUIActionBinding> Binding = FUIActionBinding::FindBinding(*this))
	{
		if (const UCommonInputSubsystem* CommonInputSubsystem = Binding->BoundWidget.IsValid()
			                                                        ? UCommonInputSubsystem::Get(
				                                                        Binding->BoundWidget->GetOwningLocalPlayer())
			                                                        : nullptr)
		{
			...
			for (const FUIActionKeyMapping& HoldMapping : Binding->HoldMappings)
			{
					...
					(!bIsGamepadKey && !bIsTouch && CurrentInputType == ECommonInputType::MouseAndKeyboard))
				{
					...
					return FText::Format(
						NSLOCTEXT("UIActionRouter", "HoldActionNameFormat", "{0} (Hold)"), Binding->ActionDisplayName);
				}
			}
		}
...
FUIInputConfig::FUIInputConfig(ECommonInputMode InInputMode, EMouseCaptureMode InMouseCaptureMode,
                               bool bInHideCursorDuringViewportCapture)
...
FUIInputConfig::FUIInputConfig(ECommonInputMode InInputMode, EMouseCaptureMode InMouseCaptureMode,
                               EMouseLockMode InMouseLockMode, bool bInHideCursorDuringViewportCapture)
	: InputMode(InInputMode)
	  , MouseCaptureMode(InMouseCaptureMode)
...
EProcessHoldActionResult FActionRouterBindingCollection::ProcessHoldInput(
	ECommonInputMode ActiveInputMode, FKey Key, EInputEvent InputEvent) const
{
		...
		if (TSharedPtr<FUIActionBinding> Binding = FUIActionBinding::FindBinding(BindingHandle))
		{
			if (ActiveInputMode == ECommonInputMode::All || ActiveInputMode == Binding->InputMode)
			{
				for (const FUIActionKeyMapping& HoldMapping : Binding->HoldMappings)
				{
					...
					if (HoldMapping.Key == Key && (bIsDisplayedPersistentAction || IsWidgetReachableForInput(
						Binding->BoundWidget.Get())))
					{
						...
						else if (Binding->IsHoldActive())
						{
							...
							else if (InputEvent == IE_Released)
							{
								...
								UE_LOG(LogUIActionRouter, VeryVerbose, TEXT("Hold released: %s after %f"),
								       *Binding->ActionName.ToString(), SecondsHeld);
								...
								{
									Binding->CancelHold();
									Binding->BeginRollback(HoldMapping.HoldRollbackTime, HoldMapping.HoldTime,
									                       Binding->Handle);
								}

								static constexpr float PressToHoldThreshold = 0.25f;
...
bool FActionRouterBindingCollection::ProcessNormalInput(ECommonInputMode ActiveInputMode, FKey Key,
                                                        EInputEvent InputEvent) const
{
	for (FUIActionBindingHandle BindingHandle : ActionBindings)
	{
		if (TSharedPtr<FUIActionBinding> Binding = FUIActionBinding::FindBinding(BindingHandle))
		{
			if (ActiveInputMode == ECommonInputMode::All || ActiveInputMode == Binding->InputMode)
			{
				auto TryConsumeInput = [&](const FKey& InKey, const UInputAction* InInputAction)
				{
					// A persistent displayed action skips the normal rules for reachability, since it'll always appear in a bound action bar
					const bool bIsDisplayedPersistentAction = Binding->bIsPersistent && Binding->bDisplayInActionBar;
					if (InKey == Key && Binding->InputEvent == InputEvent && (bIsDisplayedPersistentAction ||
						IsWidgetReachableForInput(Binding->BoundWidget.Get())))
					{
						...
						if (InInputAction)
						{
							if (TObjectPtr<const UCommonInputMetadata> CommonInputMetadata =
								CommonUI::GetEnhancedInputActionMetadata(InInputAction))
							{
								...
								if (!CommonInputMetadata->bIsGenericInputAction)
								{
									...
									CommonUI::InjectEnhancedInputForAction(
										GetActionRouter().GetLocalPlayerChecked(), InInputAction, RawValue);
				...
				if (CommonUI::IsEnhancedInputSupportEnabled() && Binding->InputAction.IsValid())
				{
					if (const UInputAction* InputAction = Binding->InputAction.Get())
					{
						...
						CommonUI::GetEnhancedInputActionKeys(GetActionRouter().GetLocalPlayerChecked(), InputAction,
						                                     InputActionKeys);
...
FActivatableTreeNode::FActivatableTreeNode(UCommonUIActionRouterBase& OwningRouter,
...
FActivatableTreeNode::FActivatableTreeNode(UCommonUIActionRouterBase& OwningRouter,
                                           UCommonActivatableWidget& ActivatableWidget,
                                           const FActivatableTreeNodeRef& InParent)
...
EProcessHoldActionResult FActivatableTreeNode::ProcessHoldInput(ECommonInputMode ActiveInputMode, FKey Key,
                                                                EInputEvent InputEvent) const
{
	if (IsReceivingInput())
	{
...
{
	InActivatableWidget.OnSlateReleased().AddSP(this, &FActivatableTreeNode::HandleChildSlateReleased,
	                                            &InActivatableWidget);

	FActivatableTreeNodeRef NewNode = MakeShareable(
		new FActivatableTreeNode(GetActionRouter(), InActivatableWidget, SharedThis(this)));
	NewNode->Init();
...
void FActivatableTreeNode::CacheFocusRestorationTarget()
{
	...
	UCommonActivatableWidget* FocusedActivatableWidget = FocusedWidget
		                                                     ? UCommonUIActionRouterBase::FindOwningActivatable(
			                                                     FocusedWidget,
			                                                     GetActionRouter().GetLocalPlayerChecked())
		                                                     : nullptr;

	if (FocusedWidget != FocusRestorationTarget.Pin() && (!FocusedActivatableWidget || IsWidgetInNodeHierarchy(
		FocusedActivatableWidget, *this)))
	{
...
{
	TOptional<FUIInputConfig> DesiredConfig = ensure(RepresentedWidget.IsValid())
		                                          ? RepresentedWidget->GetDesiredInputConfig()
...
{
	UCommonInputActionDomain* ActionDomain = ensure(RepresentedWidget.IsValid())
		                                         ? RepresentedWidget->GetCalculatedActionDomain()
		                                         : nullptr;
	const bool bHasActionDomainConfig = ActionDomain && ActionDomain->bUseActionDomainDesiredInputConfig;
	return bHasActionDomainConfig
		       ? FUIInputConfig(ActionDomain->InputMode, ActionDomain->MouseCaptureMode)
		       : TOptional<FUIInputConfig>();
}

FActivationMetadata FActivatableTreeNode::FindActivationMetadata() const
{
	TOptional<FActivationMetadata> DesiredConfig = ensure(RepresentedWidget.IsValid())
		                                               ? RepresentedWidget->GetActivationMetadata()
		                                               : TOptional<FActivationMetadata>();
	if (!DesiredConfig.IsSet() && Parent.IsValid())
	{
...
void FActivatableTreeNode::AddInputPreprocessor(const TSharedRef<IInputProcessor>& InputPreprocessor,
                                                const FInputPreprocessorRegistrationKey& RegistrationInfo)
{
...
void FActivatableTreeNode::DebugDumpRecursive(FString& OutputStr, int32 Depth, bool bIncludeActions,
                                              bool bIncludeChildren, bool bIncludeInactive) const
{
...
void FActivatableTreeNode::HandleChildSlateReleased(UCommonActivatableWidget* ChildWidget)
{
	...
	const int32 ChildToRemoveIdx = Children.IndexOfByPredicate([ChildWidget](const FActivatableTreeNodeRef& Node)
	{
		return Node->GetWidget() == ChildWidget;
	});
...
FActivatableTreeNodePtr FindLeafmostActiveChild(const FActivatableTreeNodeRef& CurNode)
{
	...
	for (const FActivatableTreeNodeRef& ChildNode : CurNode->GetChildren())
	{
		...
		if (Candidate && (!LeafmostActiveChild || Candidate->GetLastPaintLayer() > LeafmostActiveChild->
			GetLastPaintLayer()))
		{
...
FActivatableTreeRootRef FActivatableTreeRoot::Create(UCommonUIActionRouterBase& OwningRouter,
                                                     UCommonActivatableWidget& ActivatableWidget)
{
...
bool FActivatableTreeRoot::UpdateLeafmostActiveNode(FActivatableTreeNodePtr BaseCandidateNode, bool bInApplyConfig)
{
	...
	if (!bValidBaseCandidate)
	{
		...
		UE_LOG(LogUIActionRouter, Log,
		       TEXT(
			       "[FActivatableTreeRoot::UpdateLeafmostActiveNode] Node for %s is our base candidate, but isn't receiving input. This probably means its widget was Deactivated in response to the events in its NativeOnActivated: harmless if intentional."
		       ),
	...
	FActivatableTreeNodePtr NewLeafmostNode = BaseCandidateNode
		                                          ? FindLeafmostActiveChild(BaseCandidateNode.ToSharedRef())
		                                          : nullptr;
	...
	if (NewLeafmostNode != LeafmostActiveNode)
	{
		...
		if (LeafmostActiveNode.IsValid())
		{
			...
			{
				NewLeafmostWidget->OnRequestRefreshFocus().AddSP(
					this, &FActivatableTreeRoot::HandleRequestRefreshLeafmostFocus);
			}
		}
...
void FActivatableTreeRoot::ApplyLeafmostNodeConfig()
{
	...
	if (FActivatableTreeNodePtr PinnedLeafmostNode = LeafmostActiveNode.Pin())
	{
		...
		{
			UE_LOG(LogUIActionRouter, Display, TEXT("Applying input config for leaf-most node [%s]"),
			       *PinnedLeafmostNode->GetWidget()->GetName());
		...
		{
			UE_LOG(LogUIActionRouter, Log,
			       TEXT("Didn't apply input config for leaf-most node [%s] because it's not receiving input right now"),
			       *PinnedLeafmostNode->GetWidget()->GetName());
		}
	}
}
...
void FActivatableTreeRoot::FocusLeafmostNode()
{
	...
	if (TSharedPtr<SWidget> AutoRestoreTarget = LeafWidget->AutoRestoresFocus()
		                                            ? PinnedLeafmostNode->GetFocusFallbackTarget()
		                                            : nullptr)
	{
	...
	{
		UE_LOG(LogUIActionRouter, Display, TEXT("[User %d] Focused desired target %s"), OwnerSlateId,
		       *DesiredTarget->GetName());
	...
	else
	{
		...
		TSharedPtr<SWidget> PendingFocusRecipient = GetActionRouter().GetLocalPlayerChecked()->GetSlateOperations().
		                                                              GetUserFocusRecepient();
		if ((PendingFocusRecipient && PinnedLeafmostNode->IsExclusiveParentOfWidget(PendingFocusRecipient)) ||
			PinnedLeafmostNode->IsExclusiveParentOfWidget(FocusedWidget))
		{
			UE_LOG(LogUIActionRouter, Display,
			       TEXT(
			       ...
			       ), GetActionRouter().GetLocalPlayerIndex(), *LeafWidget->GetName());
		}
		else
		{
			if (LeafWidget->IsFocusable())
			{
				UE_LOG(LogUIActionRouter, Display,
				       ...
				       ), OwnerSlateId, *LeafWidget->GetName());
				LeafWidget->SetFocus();
			}
			else if (UGameViewportClient* ViewportClient = GetActionRouter().GetLocalPlayerChecked()->ViewportClient)
			{
				...
				bShouldCancelDelayedFocusOperation = FSlateApplication::Get().SetUserFocus(
					OwnerSlateId, ViewportClient->GetGameViewportWidget());
			}
		}
	}
...
void FActivatableTreeRoot::DebugDump(FString& OutputStr, bool bIncludeActions, bool bIncludeChildren,
                                     bool bIncludeInactive) const
{
