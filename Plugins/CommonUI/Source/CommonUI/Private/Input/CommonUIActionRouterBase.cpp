...
static FAutoConsoleVariableRef CVarTraceInputConfig(
	...
	bTraceInputConfig,
	TEXT(
		"Trace Input Config transitions (Non-shipping). Suggest use with Slate.Debug.TraceNavigationConfig (Non-shipping)."));
...
UCommonActivatableWidget* UCommonUIActionRouterBase::FindOwningActivatable(
	TSharedPtr<SWidget> Widget, ULocalPlayer* OwningLocalPlayer)
{
	...
	while (Widget && !OwningActivatable)
	{
		...
		{
			if (UCommonActivatableWidget* CandidateActivatable = Cast<UCommonActivatableWidget>(
				StaticCastSharedPtr<SObjectWidget>(Widget)->GetWidgetObject()))
			{
	  ...
	  , InputConfigSources(TraceInputConfigNum, "None")
{
	...
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		UConsole::RegisterConsoleAutoCompleteEntries.AddUObject(
			this, &UCommonUIActionRouterBase::PopulateAutoCompleteEntries);
	}
}

FUIActionBindingHandle UCommonUIActionRouterBase::RegisterUIActionBinding(
	const UWidget& Widget, const FBindUIActionArgs& BindActionArgs)
{
...
bool UCommonUIActionRouterBase::RegisterLinkedPreprocessor(const UWidget& Widget,
                                                           const TSharedRef<IInputProcessor>& InputPreprocessor,
                                                           int32 DesiredIndex)
{
	return RegisterLinkedPreprocessor(Widget, InputPreprocessor,
	                                  FInputPreprocessorRegistrationKey{EInputPreProcessorType::Game, DesiredIndex});
}
                                                           ...
                                                           const FInputPreprocessorRegistrationKey& RegistrationInfo)
{
	...
	if (Widget.GetCachedWidget())
	{
		...
		if (!bAlreadyExists)
		{
			PendingRegistration.InputPreProcessors.Add(FInputPreprocessorRegistration{
				RegistrationInfo, InputPreprocessor
			});
		}
...
void UCommonUIActionRouterBase::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	UCommonActivatableWidget::OnRebuilding.AddUObject(
		this, &UCommonUIActionRouterBase::HandleActivatableWidgetRebuilding);
	FCoreUObjectDelegates::GetPostGarbageCollect().AddUObject(
		this, &UCommonUIActionRouterBase::HandlePostGarbageCollect);
	...
	if (FSlateApplication::IsInitialized())
	{
		if (ensure(InputSubsystem))
		{
			...
			if (bCheckGameViewportClientValid && !GEngine->GameViewportClientClass->IsChildOf<
				UCommonGameViewportClient>())
			{
				UE_LOG(LogUIActionRouter, Error,
				       TEXT(
					       "Using CommonUI without a CommonGameViewportClient derived game viewport client. CommonUI Input routing will not function correctly.\n"
				       )
				       TEXT(
		...
		FSlateApplication::Get().OnFocusChanging().AddUObject(
			this, &UCommonUIActionRouterBase::HandleSlateFocusChanging);
	}
}
...
void UCommonUIActionRouterBase::RegisterAnalogCursorTick()
{
	if (GEngine->GameViewportClientClass->IsChildOf<UCommonGameViewportClient>())
	{
		FSlateApplication::Get().RegisterInputPreProcessor(AnalogCursor,
		                                                   UCommonUIInputSettings::Get().GetAnalogCursorSettings().
		                                                   PreprocessorRegistrationInfo);
	}
	...
	if (bIsActivatableTreeEnabled)
	{
		...
		TickHandle = FTSTicker::GetCoreTicker().AddTicker(
			FTickerDelegate::CreateUObject(this, &UCommonUIActionRouterBase::Tick));
	}
}
...
bool UCommonUIActionRouterBase::ShouldCreateSubsystem(UObject* Outer) const
{
	...
	UE_LOG(LogUIActionRouter, Display, TEXT("Found %i derived classes when attemping to create action router (%s)"),
	       ChildClasses.Num(), *GetClass()->GetName());
...
ERouteUIInputResult UCommonUIActionRouterBase::ProcessInput(FKey Key, EInputEvent InputEvent) const
{
	...
	if (GIsPlayInEditorWorld && InputEvent == IE_Pressed)
	{
		...
		TSharedPtr<FUICommandInfo> StopCommand = FInputBindingManager::Get().FindCommandInContext(
			"PlayWorld", "StopPlaySession");
		if (ensure(StopCommand))
		{
			const FModifierKeysState ModifierKeys = FSlateApplication::Get().GetModifierKeys();
			const FInputChord CheckChord(Key, EModifierKey::FromBools(ModifierKeys.IsControlDown(),
...
void UCommonUIActionRouterBase::FlushInput()
{
	...
	{
		const EProcessHoldActionResult ProcessHoldResult = PersistentActions->ProcessHoldInput(
			ActiveMode, HeldKey, IE_Released);
		if (bIsActivatableTreeEnabled && ActiveRootNode && ProcessHoldResult == EProcessHoldActionResult::Unhandled)
		{
			ActiveRootNode->ProcessHoldInput(ActiveMode, HeldKey, IE_Released);
		}
	}
...
bool UCommonUIActionRouterBase::IsWidgetInActiveRoot(const UCommonActivatableWidget* Widget) const
{
	if (Widget && ActiveRootNode)
	{
		...
		while (WidgetWalker)
		{
			...
			{
				if (UCommonActivatableWidget* CandidateActivatable = Cast<UCommonActivatableWidget>(
					StaticCastSharedPtr<SObjectWidget>(WidgetWalker)->GetWidgetObject()))
				{
...
void UCommonUIActionRouterBase::RemoveBinding(FUIActionBindingHandle Handle)
{
	if (TSharedPtr<FUIActionBinding> Binding = FUIActionBinding::FindBinding(Handle))
	{
		...
		else if (FPendingWidgetRegistration* PendingRegistration = PendingWidgetRegistrations.FindByKey(
...
void UCommonUIActionRouterBase::HandleRootWidgetSlateReleased(TWeakPtr<FActivatableTreeRoot> WeakRoot)
{
	...
	else
	{
		...
		if (NumRemoved <= 0)
		{
			...
			ensureAlwaysMsgf(false, TEXT("Root node could not be found during deactivation [%s]"),
			                 ActivatableWidget ? *ActivatableWidget->GetName() : TEXT("Unknown"));
		}
	}
}
...
void UCommonUIActionRouterBase::HandleRootNodeActivated(TWeakPtr<FActivatableTreeRoot> WeakActivatedRoot)
{
	...
	else if (UCommonInputActionDomain* WidgetActionDomain = NodeWidget
		                                                        ? NodeWidget->GetCalculatedActionDomain()
		                                                        : nullptr)
	{
		...
		if (ActionDomainRootList && ensure(ActionDomainRootList->Contains(ActivatedRoot)))
		{
			...
			ActivatedRoot->OnLeafmostActiveNodeChanged.BindUObject(
				this, &UCommonUIActionRouterBase::HandleLeafmostActiveNodeChanged);
		...
		if (!bActivatedRootNodeExists && bIsActivatableTreeEnabled)
		{
			if (bWarnAllWidgetsDeactivated)
			{
				UE_LOG(LogUIActionRouter, Warning, TEXT("All widgets deactivated. Existing input config set: %s"),
				       ActiveInputConfig.IsSet() ? TEXT(
					       "Yes - the current input config is lingering from a deactivated widget.") : TEXT("No."));
			}
...
void UCommonUIActionRouterBase::HandleSlateFocusChanging(const FFocusEvent& FocusEvent,
...
{
	if (FocusEvent.GetCause() == EFocusCause::SetDirectly && FocusEvent.GetUser() == GetLocalPlayerIndex() &&
		ActiveRootNode && ActiveRootNode->IsExclusiveParentOfWidget(OldFocusedWidget))
	{
...
bool UCommonUIActionRouterBase::ProcessInputOnActionDomains(ECommonInputMode ActiveInputMode, FKey Key,
...
EProcessHoldActionResult UCommonUIActionRouterBase::ProcessHoldInputOnActionDomains(
...
void UCommonUIActionRouterBase::ProcessRebuiltWidgets()
{
	...
	for (TWeakObjectPtr<UCommonActivatableWidget> RebuiltWidget : RebuiltWidgetsPendingNodeAssignment)
	{
		...
		{
			if (UCommonActivatableWidget* ActivatableParent = !RebuiltWidget->IsModal()
				                                                  ? ::FindOwningActivatable(*RebuiltWidget)
				                                                  : nullptr)
			{
	...
	for (UCommonActivatableWidget* RootWidget : RootCandidates)
	{
		...
		RootWidget->OnSlateReleased().AddUObject(this, &UCommonUIActionRouterBase::HandleRootWidgetSlateReleased,
		                                         WeakRoot);
	...
	while (WidgetsByDirectParent.Num() > 0 && NumWidgetsLeft != WidgetsByDirectParent.Num())
	{
		...
		for (const TPair<UCommonActivatableWidget*, TArray<UCommonActivatableWidget*>>& ParentChildrenPair :
		     WidgetsByDirectParent)
		{
	...
	for (const FPendingWidgetRegistration& PendingRegistration : PendingWidgetRegistrations)
	{
		...
		if (Widget && Widget->GetCachedWidget().IsValid())
		{
			...
			if ((PendingRegistration.bIsScrollRecipient || PendingRegistration.InputPreProcessors.Num() > 0) &&
				ensureMsgf(OwnerNode,
				           TEXT(
					           "Widget [%s] does not have a parent activatable widget at any level - cannot register preprocessors or as a scroll recipient"
				           ), *Widget->GetName()))
			{
				...
				{
					OwnerNode->AddInputPreprocessor(PreprocessorInfo.InputProcessor.ToSharedRef(),
...
void UCommonUIActionRouterBase::AssembleTreeRecursive(const FActivatableTreeNodeRef& CurNode,
                                                      TMap<UCommonActivatableWidget*, TArray<UCommonActivatableWidget*>>
                                                      & WidgetsByDirectParent)
{
...
bool UCommonUIActionRouterBase::Tick(float DeltaTime)
{
	...
	{
		const EProcessHoldActionResult ProcessHoldResult = PersistentActions->ProcessHoldInput(
			ActiveMode, HeldKey, IE_Repeat);
		if (bIsActivatableTreeEnabled && ActiveRootNode && ProcessHoldResult == EProcessHoldActionResult::Unhandled)
		{
			if (ActiveRootNode->ProcessHoldInput(ActiveMode, HeldKey, IE_Repeat) == EProcessHoldActionResult::Unhandled)
			{
				ProcessHoldInputOnActionDomains(ActiveMode, HeldKey, IE_Repeat);
			}
		}
	}
...
void UCommonUIActionRouterBase::OnShowDebugInfo(AHUD* HUD, UCanvas* Canvas, const FDebugDisplayInfo& DisplayInfo,
                                                float& YL, float& YPos)
{
	...
	DisplayDebugManager.DrawString(FString::Printf(
		TEXT("Action Router - Player [%d]: Input Type[%s]"), ControllerId,
		*InputTypeEnum->GetNameStringByValue(static_cast<int64>(CurrentInputType))));
	if (ActiveInputConfig.IsSet())
	{
		FString InputModeStr = InputModeEnum->GetNameStringByValue(
			static_cast<int64>(ActiveInputConfig->GetInputMode()));
		FString MouseCaptureStr = MouseCaptureModeEnum->GetNameStringByValue(
			static_cast<int64>(ActiveInputConfig->GetMouseCaptureMode()));

		DisplayDebugManager.DrawString(
			FString::Printf(TEXT("    Input Mode [%s] Mouse Capture [%s]"), *InputModeStr, *MouseCaptureStr));
	}
	else
	{
...
bool UCommonUIActionRouterBase::CanProcessNormalGameInput() const
{
	if (GetActiveInputMode() == ECommonInputMode::Menu)
	{
		...
		{
			return LocalPlayer.ViewportClient && SlateUser->DoesWidgetHaveCursorCapture(
...
void UCommonUIActionRouterBase::RegisterWidgetBindings(const FActivatableTreeNodePtr& TreeNode,
                                                       const TArray<FUIActionBindingHandle>& BindingHandles)
{
	for (FUIActionBindingHandle Handle : BindingHandles)
	{
		if (TSharedPtr<FUIActionBinding> Binding = FUIActionBinding::FindBinding(Handle))
		{
			if (Binding->bIsPersistent)
			{
				...
				TreeNode,
				TEXT(
					"Widget [%s] does not have a parent activatable widget at any level - cannot register standard binding to action [%s]. UserWidget parent(s): %s"
				),
...
void UCommonUIActionRouterBase::SetActiveRoot(FActivatableTreeRootPtr NewActiveRoot)
{
	...
	else
	{
		...
		if (NewActiveRoot)
		{
			...
			NewActiveRoot->OnLeafmostActiveNodeChanged.BindUObject(
				this, &UCommonUIActionRouterBase::HandleLeafmostActiveNodeChanged);
		}
	}
...
void UCommonUIActionRouterBase::UpdateLeafNodeAndConfig(FActivatableTreeRootPtr DesiredRoot,
                                                        FActivatableTreeNodePtr DesiredLeafNode)
{
...
UCommonUIActionRouterBase::FPendingWidgetRegistration& UCommonUIActionRouterBase::GetOrCreatePendingRegistration(
...
FActivatableTreeNodePtr UCommonUIActionRouterBase::FindNode(const UCommonActivatableWidget* Widget) const
{
	if (Widget)
	{
		...
		for (const TPair<TObjectPtr<UCommonInputActionDomain>, FActionDomainSortedRootList>& Pair :
		     ActionDomainRootNodes)
		{
...
FActivatableTreeNodePtr UCommonUIActionRouterBase::FindNodeRecursive(const FActivatableTreeNodePtr& CurrentNode,
                                                                     const UCommonActivatableWidget& Widget) const
{
	FActivatableTreeNodePtr FoundNode;
	if (CurrentNode)
	{
...
FActivatableTreeNodePtr UCommonUIActionRouterBase::FindNodeRecursive(const FActivatableTreeNodePtr& CurrentNode,
                                                                     const TSharedPtr<SWidget>& Widget) const
{
	FActivatableTreeNodePtr FoundNode;
	if (CurrentNode)
	{
		TSharedPtr<SWidget> CachedWidget = CurrentNode->GetWidget()
			                                   ? CurrentNode->GetWidget()->GetCachedWidget()
			                                   : nullptr;
...
void UCommonUIActionRouterBase::SetActiveUIInputConfig(const FUIInputConfig& NewConfig, const UObject* InConfigSource)
{
	...
	if (bTraceInputConfig && InConfigSource && (!ActiveInputConfig.IsSet() || NewConfig != ActiveInputConfig.
		GetValue()))
	{
...
void UCommonUIActionRouterBase::ApplyUIInputConfig(const FUIInputConfig& NewConfig, bool bForceRefresh)
{
	...
	{
		UE_LOG(LogUIActionRouter, Display, TEXT("UIInputConfig being changed. bForceRefresh: %d"),
		       bForceRefresh ? 1 : 0);
		UE_LOG(LogUIActionRouter, Display, TEXT("\tInputMode: Previous (%s), New (%s)"),
		       ActiveInputConfig.IsSet() ? *StaticEnum<ECommonInputMode>()->GetValueAsString(ActiveInputConfig->
			       GetInputMode()) : TEXT("None"),
		...
		if (UGameViewportClient* GameViewportClient = LocalPlayer.ViewportClient)
		{
			if (TSharedPtr<SViewport> ViewportWidget = GameViewportClient->GetGameViewportWidget())
			{
				if (APlayerController* PC = LocalPlayer.GetPlayerController(GetWorld()))
				{
					...
					if (bAutoFlushPressedKeys && NewConfig.GetInputMode() == ECommonInputMode::Menu && PreviousInputMode
						!= NewConfig.GetInputMode())
					{
					...
					case EMouseCaptureMode::CapturePermanently_IncludingInitialMouseDown:
						{
							PC->SetShowMouseCursor(
								ShouldAlwaysShowCursor() || !NewConfig.HideCursorDuringViewportCapture());
							...
							if (GameViewportClient->ShouldAlwaysLockMouse() || GameViewportClient->LockDuringCapture()
					...
					if (!bForceRefresh && bWasCursorHidden && PC->ShouldShowMouseCursor())
					{
						...
						if (bCenterCursor)
						{
							...
							{
								FGeometry PlayerViewGeometry = GameLayerManager->GetPlayerWidgetHostGeometry(
									&LocalPlayer);
								const FVector2D AbsoluteViewCenter = PlayerViewGeometry.
				...
				{
					UE_LOG(LogUIActionRouter, Warning,
					       TEXT("\tFailed to commit change! Local player controller was null."));
				}
			}
			else
			{
		...
		for (int32 CurrIdx = 0; CurrIdx < LocalPlayers.Num(); ++CurrIdx)
		{
			...
			if (UCommonUIActionRouterBase* ActionRouter = LocalPlayer->GetSubsystem<UCommonUIActionRouterBase>())
			{
				...
				if (ActionRouter->ActiveRootNode)
				{
					...
					ActionRouter->ActiveRootNode->DebugDump(TreeOutputStr, bIncludeActions, bIncludeChildren,
					                                        bIncludeInactive);
				...
				UE_LOG(LogUIActionRouter, Display,
				       TEXT("Dumping ActivatableWidgetTree for LocalPlayer [User %d, ControllerId %d]:\n\n%s\n"),
		...
		for (int32 i = 0; i < LocalPlayers.Num(); ++i)
		{
			if (ULocalPlayer* LocalPlayer = LocalPlayers[i])
			{
				...
				if (UCommonUIActionRouterBase* ActionRouter = LocalPlayer->GetSubsystem<UCommonUIActionRouterBase>())
				{
					...
					{
						FString InputModeStr = InputModeEnum->GetNameStringByValue(
							static_cast<int64>(ActionRouter->ActiveInputConfig->GetInputMode()));
						FString MouseCaptureStr = MouseCaptureModeEnum->GetNameStringByValue(
							static_cast<int64>(ActionRouter->ActiveInputConfig->GetMouseCaptureMode()));
						FString HideStr = ActionRouter->ActiveInputConfig->HideCursorDuringViewportCapture()
							                  ? TEXT("Yes")
							                  : TEXT("No");
						OutStr += FString::Printf(
							TEXT(
								"\tLocalPlayer[User %d, ControllerId %d] ActiveInputConfig: Input Mode [%s] Mouse Capture [%s] Hide Cursor During Capture [%s]\n"),
							i, ControllerId, *InputModeStr, *MouseCaptureStr, *HideStr);
					}
					else
					{
						OutStr += FString::Printf(
							TEXT("LocalPlayer [User %d, ControllerId %d] no ActiveInputConfig\n"), i, ControllerId);
					}
				}
				else
				{
					OutStr += FString::Printf(
						TEXT("LocalPlayer [User %d, Controller %d] has no ActionRouter\n"), i, ControllerId);
				}
			}
		}
