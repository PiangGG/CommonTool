	  ...
	  , bInteractionEnabled(true)
{
	if (bUseTransparentButtonStyleAsDefault)
	{
		...
		static const FButtonStyle* TransparentButtonStyle = new FButtonStyle(
			FUMGCoreStyle::Get().GetWidgetStyle<FButtonStyle>("NoBorder"));
...
{
	return WidgetTree->ConstructWidget<UCommonButtonInternalBase>(UCommonButtonInternalBase::StaticClass(),
...
void UCommonButtonBase::BindTriggeringInputActionToClick()
{
	...
	if (CommonUI::IsEnhancedInputSupportEnabled() && TriggeringEnhancedInputAction && !TriggeringBindingHandle.
		IsValid())
	{
		FBindUIActionArgs BindArgs(TriggeringEnhancedInputAction, false,
		                           FSimpleDelegate::CreateUObject(
			                           this, &UCommonButtonBase::HandleTriggeringActionCommited));
	...
	{
		FBindUIActionArgs BindArgs(TriggeringInputAction, false,
		                           FSimpleDelegate::CreateUObject(
			                           this, &UCommonButtonBase::HandleTriggeringActionCommited));
		BindArgs.OnHoldActionProgressed.BindUObject(this, &UCommonButtonBase::NativeOnActionProgress);
		BindArgs.OnHoldActionPressed.BindUObject(this, &UCommonButtonBase::NativeOnPressed);
...
{
	ESlateVisibility Vis = GetVisibility();
	// hidden or collapsed should have 'bInteractionEnabled' set false, but sometimes they don't :(
	return GetIsEnabled() && bButtonEnabled && bInteractionEnabled && (Vis != ESlateVisibility::Collapsed) && (Vis !=
...
void UCommonButtonBase::UpdateInputActionWidget()
{
	...
	if (GetGameInstance() && InputActionWidget)
	{
		...
		{
			UInputAction* DefaultEnhancedClickAction = bIsEnhancedInputSupportEnabled
...
void UCommonButtonBase::HandleButtonPressed()
{
	...
	if (CommonInputSubsystem && CommonInputSubsystem->GetCurrentInputType() == ECommonInputType::Touch &&
		bSimulateHoverOnTouchInput && UE::CommonUI::Private::bEnableSimulateHoverOnTouchInput)
	{
	...
	if (bRequiresHold && HoldTime > 0.f)
	{
		...
		HoldTickerHandle = FTSTicker::GetCoreTicker().AddTicker(
			FTickerDelegate::CreateUObject(this, &UCommonButtonBase::NativeOnHoldProgress));
		if (HoldProgressRollbackTickerHandle.IsValid())
		{
			FTSTicker::GetCoreTicker().RemoveTicker(HoldProgressRollbackTickerHandle);
			HoldProgressRollbackTickerHandle = nullptr;
...
void UCommonButtonBase::HandleButtonReleased()
{
	...
	if (bRequiresHold && HoldTime > 0.f)
	{
		...
		else
		{
			...
			HoldProgressRollbackTickerHandle = FTSTicker::GetCoreTicker().AddTicker(
				FTickerDelegate::CreateUObject(this, &UCommonButtonBase::NativeOnHoldProgressRollback));
...
void UCommonButtonBase::BuildStyles()
{
	if (const UCommonButtonStyle* CommonButtonStyle = GetStyleCDO())
	{
		...
		if (CommonButtonStyle->bSingleMaterial)
		{
			...
			UMaterialInterface* const BaseMaterial = Cast<UMaterialInterface>(
				DynamicSingleMaterialBrush.GetResourceObject());
		...
		NormalStyle.Normal = CommonButtonStyle->bSingleMaterial
			                     ? DynamicSingleMaterialBrush
			                     : CommonButtonStyle->NormalBase;
		NormalStyle.Hovered = CommonButtonStyle->bSingleMaterial
			                      ? DynamicSingleMaterialBrush
			                      : CommonButtonStyle->NormalHovered;
		NormalStyle.Pressed = CommonButtonStyle->bSingleMaterial
			                      ? DynamicSingleMaterialBrush
			                      : CommonButtonStyle->NormalPressed;
		...
		NormalStyle.PressedSlateSound = bHasPressedSlateSoundOverride
			                                ? PressedSlateSoundOverride
			                                : CommonButtonStyle->PressedSlateSound;
		NormalStyle.HoveredSlateSound = bHasHoveredSlateSoundOverride
			                                ? HoveredSlateSoundOverride
			                                : CommonButtonStyle->HoveredSlateSound;

		SelectedStyle.Normal = CommonButtonStyle->bSingleMaterial
			                       ? DynamicSingleMaterialBrush
			                       : CommonButtonStyle->SelectedBase;
		SelectedStyle.Hovered = CommonButtonStyle->bSingleMaterial
			                        ? DynamicSingleMaterialBrush
			                        : CommonButtonStyle->SelectedHovered;
		SelectedStyle.Pressed = CommonButtonStyle->bSingleMaterial
			                        ? DynamicSingleMaterialBrush
			                        : CommonButtonStyle->SelectedPressed;
...
void UCommonButtonBase::SetInputActionProgressMaterial(const FSlateBrush& InProgressMaterialBrush,
                                                       const FName& InProgressMaterialParam)
{
	if (InputActionWidget)
	{
...
{
	const bool bBuildStyles = InPressedSlateSoundOverride.GetResourceObject() != PressedSlateSoundOverride.
		GetResourceObject();
...
{
	const bool bBuildStyles = InHoveredSlateSoundOverride.GetResourceObject() != HoveredSlateSoundOverride.
		GetResourceObject();
...
{
	const bool bBuildStyles = InSelectedPressedSlateSoundOverride.GetResourceObject() !=
		SelectedPressedSlateSoundOverride.GetResourceObject();
...
{
	const bool bBuildStyles = InSelectedHoveredSlateSoundOverride.GetResourceObject() !=
		SelectedHoveredSlateSoundOverride.GetResourceObject();
...
{
	const bool bBuildStyles = InLockedPressedSlateSoundOverride.GetResourceObject() != LockedPressedSlateSoundOverride.
		GetResourceObject();
...
{
	const bool bBuildStyles = InLockedHoveredSlateSoundOverride.GetResourceObject() != LockedHoveredSlateSoundOverride.
		GetResourceObject();
...
void UCommonButtonBase::UpdateInputActionWidgetVisibility()
{
	if (InputActionWidget)
	{
		...
		else if (CommonInputSubsystem && bHideInputActionWithKeyboard && CommonInputSubsystem->GetCurrentInputType() !=
