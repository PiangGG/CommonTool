...
bool UCommonTabListWidgetBase::RegisterTab(FName TabNameID, TSubclassOf<UCommonButtonBase> ButtonWidgetType,
                                           UWidget* ContentWidget, const int32 TabIndex /*= INDEX_NONE*/)
{
	...
	UCommonButtonBase* const NewTabButton = TabButtonWidgetPool.GetOrCreateInstance<
		UCommonButtonBase>(ButtonWidgetType);
	...
	const int32 NewTabIndex = (TabIndex == INDEX_NONE)
		                          ? NumRegisteredTabs
		                          : FMath::Clamp(TabIndex, 0, NumRegisteredTabs);
		...
		if (bDeferRebuildingTabList)
		{
			if (!bPendingRebuild)
			{
				...
				FTSTicker::GetCoreTicker().AddTicker(
					FTickerDelegate::CreateUObject(this, &UCommonTabListWidgetBase::DeferredRebuildTabList));
			}
		}
		else
		{
...
void UCommonTabListWidgetBase::UpdateBindings()
{
	...
	if (bIsListeningForInput)
	{
		...
		{
			NextTabActionHandle = RegisterUIActionBinding(FBindUIActionArgs(
				NextTabEnhancedInputAction, false,
				FSimpleDelegate::CreateUObject(this, &UCommonTabListWidgetBase::HandleNextTabAction)));
		}
		else
		{
			NextTabActionHandle = RegisterUIActionBinding(FBindUIActionArgs(
				NextTabInputActionData, false,
				FSimpleDelegate::CreateUObject(this, &UCommonTabListWidgetBase::HandleNextTabAction)));
		}

		if (bIsEnhancedInputSupportEnabled && PreviousTabEnhancedInputAction)
		{
			PrevTabActionHandle = RegisterUIActionBinding(FBindUIActionArgs(
				PreviousTabEnhancedInputAction, false,
				FSimpleDelegate::CreateUObject(this, &UCommonTabListWidgetBase::HandlePreviousTabAction)));
		}
		else
		{
			PrevTabActionHandle = RegisterUIActionBinding(FBindUIActionArgs(
				PreviousTabInputActionData, false,
				FSimpleDelegate::CreateUObject(this, &UCommonTabListWidgetBase::HandlePreviousTabAction)));
		}
	}
	else
...
void UCommonTabListWidgetBase::HandleTabButtonSelected(UCommonButtonBase* SelectedTabButton, int32 ButtonIndex)
{
	for (auto& TabPair : RegisteredTabsByID)
	{
		...
		if (TabInfo.TabButton == SelectedTabButton)
		{
			...
			{
				if (ensureMsgf(TabInfo.ContentInstance,
				               TEXT(
					               "A CommonTabListWidget tab button lacks a tab content widget to set its linked switcher to."
				               )) &&
					ensureMsgf(LinkedSwitcher.IsValid(),
					           TEXT(
						           "A CommonTabListWidgetBase.has a registered tab with a content widget to switch to, but has no linked activatable widget switcher. Did you forget to call SetLinkedSwitcher to establish the association?"
					           )))
				{
...
void UCommonTabListWidgetBase::RebuildTabList()
{
	...
	SortedRegisteredTabsByID.ValueSort(
		[](const FCommonRegisteredTabInfo& TabInfoA, const FCommonRegisteredTabInfo& TabInfoB)
		{
