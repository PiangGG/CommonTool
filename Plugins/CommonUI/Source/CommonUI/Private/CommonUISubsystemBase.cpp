...
FSlateBrush UCommonUISubsystemBase::GetInputActionButtonIcon(const FDataTableRowHandle& InputActionRowHandle,
                                                             ECommonInputType InputType, const FName& GamepadName) const
{
	if (ensure(InputType != ECommonInputType::Count) && !InputActionRowHandle.IsNull())
	{
		...
		if (ensure(InputActionData))
		{
			...
			if (UCommonInputPlatformSettings::Get()->TryGetInputBrush(SlateBrush, InputTypeInfo.GetKey(), InputType,
			                                                          GamepadName))
			{
				return SlateBrush;
			}
...
FSlateBrush UCommonUISubsystemBase::GetEnhancedInputActionButtonIcon(const UInputAction* InputAction,
                                                                     const ULocalPlayer* LocalPlayer) const
{
	if (LocalPlayer)
	{
 ...
 * @owner chris.gagnon
 */
void UCommonUISubsystemBase::FireEvent_ButtonClicked(const FString& ButtonName, const FString& ABTestName,
                                                     const FString& ExtraData) const
{
