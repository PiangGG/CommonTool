...
void UCommonActivatableWidget::NativeConstruct()
{
	...
	if (bIsBackHandler)
	{
		...
		{
			FBindUIActionArgs BindArgs(ICommonInputModule::GetSettings().GetEnhancedInputBackAction(),
			                           FSimpleDelegate::CreateUObject(
				                           this, &UCommonActivatableWidget::HandleBackAction));
		...
		{
			FBindUIActionArgs BindArgs(ICommonInputModule::GetSettings().GetDefaultBackAction(),
			                           FSimpleDelegate::CreateUObject(
				                           this, &UCommonActivatableWidget::HandleBackAction));
			BindArgs.bDisplayInActionBar = bIsBackActionDisplayedInActionBar;
...
TOptional<FUIInputConfig> UCommonActivatableWidget::GetDesiredInputConfig() const
{
	...
	if (GetClass()->IsFunctionImplementedInScript(
		GET_FUNCTION_NAME_CHECKED(UCommonActivatableWidget, BP_GetDesiredInputConfig)))
	{
...
void UCommonActivatableWidget::SetBindVisibilities(ESlateVisibility OnActivatedVisibility,
                                                   ESlateVisibility OnDeactivatedVisibility, bool bInAllActive)
{
...
void UCommonActivatableWidget::BindVisibilityToActivation(UCommonActivatableWidget* ActivatableWidget)
{
	if (ActivatableWidget && !VisibilityBoundWidgets.Contains(ActivatableWidget))
	{
		...
		ActivatableWidget->OnActivated().AddUObject(
			this, &UCommonActivatableWidget::HandleVisibilityBoundWidgetActivations);
		ActivatableWidget->OnDeactivated().AddUObject(
			this, &UCommonActivatableWidget::HandleVisibilityBoundWidgetActivations);
...
TObjectPtr<UCommonInputActionDomain> UCommonActivatableWidget::GetCalculatedActionDomain()
{
	...
	while (CurrentWidget)
	{
		...
		{
			const TSharedPtr<ICommonInputActionDomainMetaData> Metadata = CurrentWidget->GetMetaData<
				ICommonInputActionDomainMetaData>();
			if (Metadata.IsValid())
			{
			...
			if (UCommonActivatableWidget* CurrentActivatable = Cast<UCommonActivatableWidget>(
				StaticCastSharedPtr<SObjectWidget>(CurrentWidget)->GetWidgetObject()))
			{
				if (CurrentActivatable->bOverrideActionDomain)
				{
					UCommonInputActionDomain* CurrentActionDomain =
						CurrentActivatable->GetOwningLocalPlayer() == OwningLocalPlayer
							? CurrentActivatable->ActionDomainOverride.Get()
							: nullptr;
...
void UCommonActivatableWidget::NativeOnActivated()
{
	if (ensureMsgf(
		bIsActive,
		TEXT(
			"[%s] has called NativeOnActivated, but isn't actually activated! Never call this directly - call ActivateWidget()"
		), *GetName()))
	{
		if (bSetVisibilityOnActivated)
		{
			SetVisibility(ActivatedVisibility);
			UE_LOG(LogCommonUI, Verbose, TEXT("[%s] set visibility to [%s] on activation"), *GetName(),
			       *StaticEnum<ESlateVisibility>()->GetDisplayValueAsText(ActivatedVisibility).ToString());
		}
...
void UCommonActivatableWidget::NativeOnDeactivated()
{
	if (ensure(!bIsActive))
	{
		if (bSetVisibilityOnDeactivated)
		{
			...
			UE_LOG(LogCommonUI, Verbose, TEXT("[%s] set visibility to [%s] on deactivation"), *GetName(),
			       *StaticEnum<ESlateVisibility>()->GetDisplayValueAsText(DeactivatedVisibility).ToString());
		}
...
bool UCommonActivatableWidget::NativeOnHandleBackAction()
{
	if (bIsBackHandler)
	{
		if (!BP_OnHandleBackAction())
		{
			...
			UE_LOG(LogCommonUI, Verbose,
			       TEXT("[%s] handled back with default implementation. Deactivating immediately."), *GetName());
			DeactivateWidget();
		}
		return true;
	}
...
void UCommonActivatableWidget::HandleVisibilityBoundWidgetActivations()
{
	...
	OldDeactivatedVisibility = bSetVisibilityOnActivated && IsActivated()
		                           ? ActivatedVisibility
		                           : OldDeactivatedVisibility;
	OldDeactivatedVisibility = bSetVisibilityOnDeactivated && !IsActivated()
