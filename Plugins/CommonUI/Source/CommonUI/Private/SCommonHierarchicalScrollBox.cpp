...
void SCommonHierarchicalScrollBox::AppendFocusableChildren(TArray<TSharedRef<SWidget>>& OutChildren,
...
FNavigationReply SCommonHierarchicalScrollBox::OnNavigation(const FGeometry& MyGeometry,
	...
	if (FocusedChild.IsValid())
	{
		...
		if (FocusedChildDirection != 0)
		{
			...
			if (ChildIndex >= Children.Num() || ChildIndex < 0)
			{
				...
				if (NavigationMetaData.IsValid() && NavigationMetaData->GetBoundaryRule(Type) ==
					EUINavigationRule::Wrap)
				{
			...
			for (; ChildIndex >= 0 && ChildIndex < Children.Num(); ChildIndex += FocusedChildDirection)
			{
				...
				if (PossiblyFocusableChild->SupportsKeyboardFocus())
				{
					...
					if (!bIsValidScrollDirection)
					{
						...
						const FVector2D NextChildPos = PossiblyFocusableChild->GetCachedGeometry().
						                                                       GetAbsolutePosition();
						if (Orientation == Orient_Vertical)
						{
							switch (InNavigationEvent.GetNavigationType())
