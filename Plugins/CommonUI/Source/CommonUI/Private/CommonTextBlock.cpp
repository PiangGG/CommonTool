...
void UCommonTextBlock::PostLoad()
{
	...
	if (!Style && !bStyleNoLongerNeedsConversion && !IsRunningDedicatedServer())
	{
		...
		UCommonTextStyle* DefaultStyleCDO = DefaultStyle
			                                    ? Cast<UCommonTextStyle>(DefaultStyle->ClassDefaultObject)
			                                    : nullptr;
		if (DefaultStyleCDO)
		{
			...
			// Font
			{
				...
				{
					MyFont.OutlineSettings.bSeparateFillAlpha = DefaultStyleCDO->Font.OutlineSettings.
						bSeparateFillAlpha;
					bFontHasChanged = true;
				}
				if (MyFont.OutlineSettings.bApplyOutlineToDropShadows == CDOFont.OutlineSettings.
					bApplyOutlineToDropShadows)
				{
					MyFont.OutlineSettings.bApplyOutlineToDropShadows = DefaultStyleCDO->Font.OutlineSettings.
						bApplyOutlineToDropShadows;
					bFontHasChanged = true;
				}
			...
			// Margin
			if (Margin.Left == CDO->Margin.Left) { Margin.Left = DefaultStyleCDO->Margin.Left; }
			else { bAllDefaults = false; }
			if (Margin.Top == CDO->Margin.Top) { Margin.Top = DefaultStyleCDO->Margin.Top; }
			else { bAllDefaults = false; }
			if (Margin.Right == CDO->Margin.Right) { Margin.Right = DefaultStyleCDO->Margin.Right; }
			else { bAllDefaults = false; }
			if (Margin.Bottom == CDO->Margin.Bottom) { Margin.Bottom = DefaultStyleCDO->Margin.Bottom; }
			else { bAllDefaults = false; }

			// LineHeightPercentage
			if (LineHeightPercentage == CDO->LineHeightPercentage)
			{
				LineHeightPercentage = DefaultStyleCDO->LineHeightPercentage;
			}
			else { bAllDefaults = false; }

			// ApplyLineHeightToBottomLine
			if (ApplyLineHeightToBottomLine == CDO->ApplyLineHeightToBottomLine)
			{
				ApplyLineHeightToBottomLine = DefaultStyleCDO->ApplyLineHeightToBottomLine;
			}
			else { bAllDefaults = false; }

			// ColorAndOpacity
			if (GetColorAndOpacity() == CDO->GetColorAndOpacity()) { SetColorAndOpacity(DefaultStyleCDO->Color); }
			else { bAllDefaults = false; }
...
bool UCommonTextBlock::CanEditChange(const FProperty* InProperty) const
{
	if (Super::CanEditChange(InProperty))
	{
		...
		if (const UCommonTextScrollStyle* TextScrollStyle = GetScrollStyleCDO())
		{
		...
		if (bAutoCollapseWithEmptyText && InProperty->GetFName() == GET_MEMBER_NAME_CHECKED(
			UCommonTextBlock, Visibility))
		{
			return false;
		}
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
{
	const UCommonTextScrollStyle* TextScrollStyle = GetScrollStyleCDO();
	if (!TextScrollStyle)
	{
	...
	MyTextBlock->AddMetadata<FReflectionMetaData>(
		MakeShared<FReflectionMetaData>(GetFName(), GetClass(), this, GetSourceAssetOrClass()));
#endif
