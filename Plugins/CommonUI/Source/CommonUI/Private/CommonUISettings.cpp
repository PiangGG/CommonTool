...
void UCommonUISettings::LoadEditorData()
{
	if (!bDefaultDataLoaded)
	{
		...
		DefaultThrobberBrush.DrawAs = DefaultThrobberMaterialInstance
	...
	if (SimulatedPlatform != NAME_None)
	{
		...
		if (SimulatedPlatform != LastSimulatedPlatform)
		{
			...
			if (FConfigCacheIni* PlatformIni = FConfigCacheIni::ForPlatform(SimulatedPlatform))
			{
				...
				PlatformIni->GetArray(*GetClass()->GetPathName(),
				                      GET_MEMBER_NAME_STRING_CHECKED(UCommonUISettings, PlatformTraits), Entries,
				                      TEXT("Game"));
				...
				for (FString Entry : Entries)
				{
					...
					GameplayTagStruct->ImportText(*Entry, &Element, nullptr, PPF_None, nullptr,
					                              GameplayTagStruct->GetName(), true);
