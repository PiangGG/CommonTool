...
public:
	...
	virtual ~FCommonRichTextDecorator() override
	{
	}
	                                     ...
	                                     const ISlateStyle* Style) override final
	{
		...
		FTextRunInfo RunInfo(RunParseResult.Name,
		                     FText::FromString(OriginalText.Mid(RunParseResult.ContentRange.BeginIndex,
		                                                        RunParseResult.ContentRange.EndIndex - RunParseResult.
		                                                        ContentRange.BeginIndex)));
		...
		if (CommonUIUtils::ShouldDisplayMobileUISizes() &&
			UE::CommonUI::CommonRichTextBlock::bApplyMobileScaleOnTextRuns)
		{
...
private:
	TSharedRef<SWidget> CreateIconWidget(const FInlineIconEntry& Entry, const FTextBlockStyle& TextStyle,
	                                     float IconScale, bool bTintIcon)
	{
		...
		if (!LazyAsset.IsValid())
		{
			...
			UAssetManager::GetStreamableManager().RequestAsyncLoad(LazyAsset.ToSoftObjectPath(),
			                                                       ...
			                                                       {
				                                                       if (LocalWeakThis.IsValid() && LazyAsset.
					                                                       IsValid())
				                                                       {
		...
		const TSharedRef<FSlateFontMeasure> FontMeasure = FSlateApplication::Get().GetRenderer()->
			GetFontMeasureService();
		const float MaxTextHeight = FontMeasure->GetMaxCharacterHeight(TextStyle.Font);
	...
	TSharedRef<SWidget> CreateTextWidget(const FInlineIconEntry& Entry, const FTextBlockStyle& TextStyle,
...
protected:
	                                                  ...
	                                                  const FTextBlockStyle& TextStyle) const override
	{
		...
		if (ensure(InlineIconEntry))
		{
			...
			ERichTextInlineIconDisplayMode EntryDisplayMode = InlineIconEntryName == TEXT("cash")
				                                                  ? ERichTextInlineIconDisplayMode::TextOnly
				                                                  : Owner->InlineIconDisplayMode;
		...
		if (InlineIconsByTag.Num() == 0)
		{
			if (UCommonUIRichTextData* RichTextData = UCommonUIRichTextData::Get())
			{
				for (const TPair<FName, uint8*>& IconPair : RichTextData->GetIconMap())
				{
					...
					{
						InlineIconsByTag.Add(IconPair.Key.ToString(),
						                     FInlineIconEntry(IconData->DisplayName, IconData->ImageSize,
						                                      IconData->ResourceObject));
					}
				}
			}
		}
...
/*static*/
FString UCommonRichTextBlock::EscapeStringForRichText(FString InString)
{
...
TSharedRef<SWidget> UCommonRichTextBlock::RebuildWidget()
{
	...
	MyRichTextBlock->AddMetadata<FReflectionMetaData>(
