...
#if ALLOW_CONSOLE
	if (ViewportConsole && !ViewportConsole->ConsoleState.IsEqual(NAME_Typing) && !ViewportConsole->ConsoleState.
		IsEqual(NAME_Open))
#endif
	{
...
bool UCommonGameViewportClient::InputAxis(FViewport* InViewport, FInputDeviceId InputDevice, FKey Key, float Delta,
...
bool UCommonGameViewportClient::InputTouch(FViewport* InViewport, int32 ControllerId, uint32 Handle,
                                           ETouchType::Type Type, const FVector2D& TouchLocation, float Force,
                                           FDateTime DeviceTimestamp, uint32 TouchpadIndex)
{
#if ALLOW_CONSOLE
	if (ViewportConsole != NULL && (ViewportConsole->ConsoleState != NAME_Typing) && (ViewportConsole->ConsoleState !=
		NAME_Open))
#endif
	{
		FReply Result = FReply::Unhandled();
...
void UCommonGameViewportClient::HandleRerouteInput(FInputDeviceId DeviceId, FKey Key, EInputEvent EventType,
                                                   FReply& Reply)
{
...
void UCommonGameViewportClient::HandleRerouteTouch(int32 ControllerId, uint32 TouchId, ETouchType::Type TouchType,
                                                   const FVector2D& TouchLocation, FReply& Reply)
{
...
bool UCommonGameViewportClient::IsKeyPriorityAboveUI(const FInputKeyEventArgs& EventArgs)
{
	...
	if (ViewportConsole && ViewportConsole->InputKey(EventArgs.InputDevice, EventArgs.Key, EventArgs.Event,
