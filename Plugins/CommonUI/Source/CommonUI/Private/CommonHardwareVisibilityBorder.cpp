...
void UCommonHardwareVisibilityBorder::UpdateVisibility(UCommonUIVisibilitySubsystem* VisSystem)
{
	if (!IsDesignTime())
	{
		...
		{
			const UCommonUIVisibilitySubsystem* CommonInputSubsystem = VisSystem
				                                                           ? VisSystem
			...
			{
				UE_LOG(LogCommonUI, Verbose, TEXT("[%s] -> ULocalPlayer not available, using PlatformTraits instead"),
				       *GetName());
