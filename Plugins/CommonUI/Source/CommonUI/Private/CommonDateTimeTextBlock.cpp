...
DECLARE_CYCLE_STAT(TEXT("CommonDateTimeTextBlock UpdateUnderlyingText"),
                   STAT_CommonDateTimeTextBlock_UpdateUnderlyingText, STATGROUP_UI);
...
void UCommonDateTimeTextBlock::SetDateTimeValue(const FDateTime InDateTime, bool InShowAsCountdown,
	...
	if (CustomTimespanFormat.IsEmpty())
	{
		...
		else
		{
			...
			if (NewDaysCount > 2)
			{
				...
				{
					FText TimespanFormatPattern = NSLOCTEXT("CommonDateTimeTextBlock", "DaysFormatText",
			...
			{
				const int32 NewHoursCount = static_cast<int32>(InTimespan.GetTotalHours());
				if (NewHoursCount > 12)
				{
					if (NewHoursCount != GetLastHoursCount())
					{
						FText TimespanFormatPattern = NSLOCTEXT("CommonDateTimeTextBlock", "HoursFormatText",
