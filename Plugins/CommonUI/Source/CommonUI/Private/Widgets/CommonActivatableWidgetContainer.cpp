...
{
	if (SlateWidget && SlateWidget != SNullWidget::NullWidget && ensure(
		SlateWidget->GetType().IsEqual(TEXT("SObjectWidget"))))
	{
		UCommonActivatableWidget* ActivatableWidget = Cast<UCommonActivatableWidget>(
			StaticCastSharedPtr<SObjectWidget>(SlateWidget)->GetWidgetObject());
...
void UCommonActivatableWidgetContainerBase::RemoveWidget(UCommonActivatableWidget& WidgetToRemove)
{
	UE_LOG(LogCommonUI, VeryVerbose, TEXT("UCommonActivatableWidgetContainerBase::RemoveWidget() WidgetToRemove: %s"),
	       *WidgetToRemove.GetName());
...
TSharedRef<SWidget> UCommonActivatableWidgetContainerBase::RebuildWidget()
{
	MyOverlay = SNew(SOverlay)
		...
		[
			...
			.OnIsTransitioningChanged_UObject(
				this, &UCommonActivatableWidgetContainerBase::HandleSwitcherIsTransitioningChanged)
		]
		+ SOverlay::Slot()
		[
...
{
	UE_LOG(LogCommonUI, VeryVerbose,
	       TEXT("UCommonActivatableWidgetContainerBase::SetSwitcherIndex() TargetIndex: %d, bInstantTransition: %d"),
	       TargetIndex, bInstantTransition);
...
UCommonActivatableWidget* UCommonActivatableWidgetContainerBase::BP_AddWidget(
...
UCommonActivatableWidget* UCommonActivatableWidgetContainerBase::AddWidgetInternal(
	TSubclassOf<UCommonActivatableWidget> ActivatableWidgetClass,
	TFunctionRef<void(UCommonActivatableWidget&)> InitFunc)
{
...
{
	UE_LOG(LogCommonUI, VeryVerbose,
	       TEXT("UCommonActivatableWidgetContainerBase::RegisterInstanceInternal() NewWidget: %s"),
	       *NewWidget.GetName());
...
{
	UE_LOG(LogCommonUI, VeryVerbose,
	       TEXT("UCommonActivatableWidgetContainerBase::HandleSwitcherIsTransitioningChanged() bIsTransitioning: %d"),
	       bIsTransitioning);
...
{
	UE_LOG(LogCommonUI, VeryVerbose,
	       TEXT("UCommonActivatableWidgetContainerBase::HandleActiveWidgetDeactivated() DeactivatedWidget: %s"),
	       *DeactivatedWidget->GetName());
...
void UCommonActivatableWidgetContainerBase::ReleaseWidget(const TSharedRef<SWidget>& WidgetToRelease)
{
	...
	{
		UE_LOG(LogCommonUI, VeryVerbose, TEXT("UCommonActivatableWidgetContainerBase WidgetToRelease: %s"),
	...
	{
		UE_LOG(LogCommonUI, Warning,
		       TEXT("UCommonActivatableWidgetContainerBase::ReleaseWidget() No matching Activatable Widget found."));
	}

	const int32 RemovedIndex = MySwitcher->RemoveSlot(WidgetToRelease);
	if (RemovedIndex != INDEX_NONE)
	{
		UE_LOG(LogCommonUI, VeryVerbose, TEXT("UCommonActivatableWidgetContainerBase Widget removed from slot %d"),
		       RemovedIndex);
...
{
	UE_LOG(LogCommonUI, VeryVerbose,
	       TEXT("UCommonActivatableWidgetContainerBase::HandleActiveIndexChanged() ActiveWidgetIndex: %d"),
	       ActiveWidgetIndex);
	...
	if (DisplayedWidget)
	{
		...
		DisplayedWidget->OnDeactivated().AddUObject(
			this, &UCommonActivatableWidgetContainerBase::HandleActiveWidgetDeactivated, ToRawPtr(DisplayedWidget));
		...
		if (UWorld* MyWorld = GetWorld())
		{
			...
			TimerManager.SetTimerForNextTick(
				FTimerDelegate::CreateWeakLambda(this, [this]() { InvalidateLayoutAndVolatility(); }));
		}
	}
	else
	{
...
void UCommonActivatableWidgetQueue::OnWidgetAddedToList(UCommonActivatableWidget& AddedWidget)
{
	...
	{
		UE_LOG(LogCommonUI, VeryVerbose,
		       TEXT("UCommonActivatableWidgetQueue::OnWidgetAddedToList() PriorNumWidgets: %d"),
		       MySwitcher->GetNumWidgets() - 1);
		...
		else if (DisplayedWidget && !DisplayedWidget->IsActivated() && MySwitcher->GetNumWidgets() == 3)
		{
			...
			SetSwitcherIndex(1, true);
			//and get there fast, we need to finish and evict the old widget before anything else happens
		}
	}
}
