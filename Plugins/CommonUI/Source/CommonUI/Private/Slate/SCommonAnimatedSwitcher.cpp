...
int32 SCommonAnimatedSwitcher::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry,
                                       ...
                                       int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	return SWidgetSwitcher::OnPaint(Args, AllottedGeometry, MyCullingRect, OutDrawElements, LayerId,
	                                CompoundedWidgetStyle, bParentEnabled);
}
...
void SCommonAnimatedSwitcher::TransitionToIndex(int32 NewWidgetIndex, bool bInstantTransition)
{
	...
	{
		PendingActiveWidgetIndex = INDEX_NONE;
		UE_LOG(LogSlate, Verbose, TEXT("Called SCommonAnimatedSwitcher::TransitionToIndex('%d') to an invalid Index"),
		       NewWidgetIndex);
		return;
	}
	...
	else if (TransitionSequence.IsPlaying())
	{
		...
		const bool bNeedsReverse = (TransitionSequence.IsInReverse() && bNewGoalHigher) ||
			// Currently headed to a lower index, now need to go to a higher one
			(!TransitionSequence.IsInReverse() && bNewGoalLower) ||
			// Currently headed to a higher index, now need to go to a lower one
	...
	else if (bNewGoalHigher || bNewGoalLower)
	{
		...
		if (!bIsTransitionTimerRegistered)
		{
			...
			RegisterActiveTimer(
				0.f, FWidgetActiveTimerDelegate::CreateSP(this, &SCommonAnimatedSwitcher::UpdateTransition));
		}
	}
}
...
EActiveTimerReturnType SCommonAnimatedSwitcher::UpdateTransition(double InCurrentTime, float InDeltaTime)
{
	...
	{
		static constexpr float MaxScaleModifier = 0.25f;
	...
	{
		static constexpr float MaxTranslation = 200.f;
	...
	if (!TransitionSequence.IsPlaying())
	{
		...
		if (bTransitioningOut && (!PinnedPendingActiveWidget.IsValid() || GetWidgetIndex(
			PinnedPendingActiveWidget.ToSharedRef()) == INDEX_NONE))
		{
			// The PendingActiveWidget is no longer contained in this switcher
			UE_LOG(LogSlate, Verbose,
			       TEXT(
				       "SCommonAnimatedSwitcher Pending Widget was removed or became invalid while a transition was happening"
			       ));
...
bool SCommonAnimatedSwitcher::TryTransitionFallbackOfPendingWidget()
{
	if (IsTransitionFallbackEnabled() && GetNumWidgets() > 0)
	{
		...
		PendingActiveWidgetIndex = INDEX_NONE;
		UE_LOG(LogSlate, Verbose,
		       TEXT("SCommonAnimatedSwitcher failed to fall back to another Pending Widget for the transition"));
	}
