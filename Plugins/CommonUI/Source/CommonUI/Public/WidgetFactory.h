...
class TWidgetFactory : public FGCObject
{
	static_assert(TIsDerivedFrom<WidgetType, UUserWidget>::IsDerived,
	              "Can only use TWidgetFactory with types deriving from UUserWidget");
	...
	FORCEINLINE APlayerController* OwningObjectAsPlayerController() const
	{
		return Cast<APlayerController>(OuterGetter());
	}

public:
	using ConstructMethodType = TFunctionRef<TSharedPtr<SObjectWidget>(UUserWidget*, TSharedRef<SWidget>)>;

	TWidgetFactory() = default;

	TWidgetFactory(UWidget& InOwningWidget, TSubclassOf<WidgetType> InWidgetClass,
	               TFunction<UObject*(void)>&& InOuterGetter, bool InTrackActiveWidgets = false)
	...
	virtual ~TWidgetFactory() override
	{
		ensureMsgf(ActiveWidgets.Num() == 0,
		           TEXT(
			           "TWidgetFactory: ActiveWidgets not empty! Please check your usage: \n\tAre you overriding ReleaseSlateResources and calling Reset in your User Widget?"
		           ));
		ensureMsgf(InactiveWidgets.Num() == 0,
		           TEXT(
			           "TWidgetFactory: InactiveWidgets not empty! Please check your usage: \n\tAre you overriding ReleaseSlateResources and calling Reset in your User Widget?"
		           ));
		ensureMsgf(CachedSlateWidgets.Num() == 0,
		           TEXT(
			           "TWidgetFactory: CachedSlateWidgets not empty! Please check your usage: \n\tAre you overriding ReleaseSlateResources and calling Reset in your User Widget?"
		           ));
	}
	...
	template <class DerivedWidgetType = SObjectWidget,
	          class = typename TEnableIf<TIsDerivedFrom<DerivedWidgetType, SObjectWidget>::IsDerived,
	                                     DerivedWidgetType>::Type>
	...
	template <class DerivedWidgetType = SObjectWidget,
	          class = typename TEnableIf<TIsDerivedFrom<DerivedWidgetType, ITableRow>::IsDerived,
	                                     DerivedWidgetType>::Type>
