...
public:
	...
	FSlateBrush GetInputActionButtonIcon(const FDataTableRowHandle& InputActionRowHandle, ECommonInputType InputType,
	...
	UFUNCTION(BlueprintCallable, Category = CommonUISubsystem)
	FSlateBrush GetEnhancedInputActionButtonIcon(const UInputAction* InputAction,
	                                             const ULocalPlayer* LocalPlayer) const;

	/** Analytic Events **/

	//CommonUI.ButtonClicked
	void FireEvent_ButtonClicked(const FString& InstanceName, const FString& ABTestName,
