...
public:
	...
	void InterpolateToValue(const float TargetValue, float MaximumInterpolationDuration = 3.0f,
	                        float MinimumChangeRate = 1.0f, float OutroOffset = 0.0f);
	...
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FOnInterpolationUpdated, UCommonNumericTextBlock*, NumericTextBlock,
	                                               float, LastValue, float, NewValue);
	...
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnInterpolationEnded, UCommonNumericTextBlock*, NumericTextBlock,
	                                             const bool, HadCompleted);
...
protected:
	...
	virtual FText FormatText(float InCurrentNumericValue,
	                         const FNumberFormattingOptions& InNumberFormattingOptions) const;

private:
	FNumberFormattingOptions MakeNumberFormattingOptions() const;

	float CalculateInterpolationDuration(const float InTargetValue, const float InMinimumChangeRate,
	                                     const float InMaximumInterpolationDuration) const;

	void OnTimerTick();
	void Tick(float DeltaTime);

	void EnterNumericInterpolation(const float InitialValue, const float FinalValue, const float Duration,
	                               const float OutroOffset);
