...
public:
	...
	virtual FUIActionBindingHandle RegisterUIActionBinding(const UWidget& Widget,
	                                                       const FBindUIActionArgs& BindActionArgs);
	UE_DEPRECATED(
		5.5,
		"The version of RegisterLinkedPreprocessor taking an int32 DesiredIndex parameter is deprecated and uses EInputPreProcessorType::Game as a default. For greater control in pre-processor priority, please use the new version with a FInputPreprocessorRegistrationInfo parameter")
	...
	EMouseCaptureMode GetActiveMouseCaptureMode(
		EMouseCaptureMode DefaultMouseCapture = EMouseCaptureMode::NoCapture) const;
...
private:
	...
	void RegisterWidgetBindings(const FActivatableTreeNodePtr& TreeNode,
	...
	FActivatableTreeNodePtr FindNodeRecursive(const FActivatableTreeNodePtr& CurrentNode,
	...
	void AssembleTreeRecursive(const FActivatableTreeNodeRef& CurNode,
	...
	void HandleSlateFocusChanging(const FFocusEvent& FocusEvent, const FWeakWidgetPath& OldFocusedWidgetPath,
	...
	EProcessHoldActionResult ProcessHoldInputOnActionDomains(ECommonInputMode ActiveInputMode, FKey Key,
	...
	struct FPendingWidgetRegistration
	{
		...
		struct UE_DEPRECATED(5.5, "This struct is deprecated, please use FInputPreprocessorRegistration instead.")
			FPreprocessorRegistration
		{
			TSharedPtr<IInputProcessor> Preprocessor;
			int32 DesiredIdx = 0;

			bool operator==(const TSharedRef<IInputProcessor>& OtherPreprocessor) const
			{
				return Preprocessor == OtherPreprocessor;
			}
		};
