...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FHardwareVisibilityTagsChangedDynamicEvent, UCommonUIVisibilitySubsystem*,
                                            TagSubsystem);
...
public:
	...
	DECLARE_EVENT_OneParam(UCommonUIVisibilitySubsystem, FHardwareVisibilityTagsChangedEvent,
	                       UCommonUIVisibilitySubsystem*);
	 ...
	 */
	bool HasVisibilityTag(const FGameplayTag VisibilityTag) const
	{
		return ComputedVisibilityTags.HasTag(VisibilityTag);
	}
...
#if WITH_EDITOR
	static void SetDebugVisibilityConditions(const FGameplayTagContainer& TagsToEnable,
	                                         const FGameplayTagContainer& TagsToSuppress);
#endif
